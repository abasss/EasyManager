...
class Holiday extends CommonObject
{
	 ...
	 *    @param      boolean   		$type			If true, read ERP user list, if false, return vacation balance list.
	...
	/**
	 *	Compte le nombre d'utilisateur actifs dans ERP
	 *
	...
	/**
	 *	Compte le nombre d'utilisateur actifs dans ERP sans CP
	 *
	 *  @return     int      retourne le nombre d'utilisateur
	 */
	...
	/**
	 *  Compare le nombre d'utilisateur actif de ERP à celui des utilisateurs des congés payés
	 *
	 *  @param    int	$userMounirWithoutCP	Number of active users in ERP without holidays
	 *  @param    int	$userCP    				Number of active users into table of holidays
	 *  @return   int							<0 if KO, >0 if OK
	 */
	public function verifNbUsers($userMounirWithoutCP, $userCP)
	{
		if (empty($userCP)) $userCP=0;
		dol_syslog(get_class($this).'::verifNbUsers userMounir='.$userMounirWithoutCP.' userCP='.$userCP);
		return 1;
	}
