...
if ($action == 'confirm_send')
{
    ...
    if ($object->statut == Holiday::STATUS_DRAFT && $cancreate)
    {
        ...
        if ($verif > 0)
        {
            ...
            $message.= "- ".$langs->transnoentitiesnoconv("Link")." : ".$mounir_main_url_root."/holiday/card.php?id=".$object->id."\n\n";
            $message.= "\n";
...
if ((empty($id) && empty($ref)) || $action == 'add' || $action == 'request' || $action == 'create')
{
    ...
    else
    {
        ...
        $doleditor = new DolEditor('description', GETPOST('description', 'none'), '', 80, 'mounir_notes', 'In', 0, false, true, ROWS_3, '90%');
        ...
        if (($id > 0) || $ref)
        {
            ...
            if ($cancreate)
            {
                ...
                else
                {
                    ...
                    print '<td>'.$langs->trans('DescCP').'</td>';
                    print '<td class="tdtop">';
                    $doleditor = new DolEditor('description', $object->description, '', 80, 'mounir_notes', 'In', 0, false, true, ROWS_3, '90%');
                    print $doleditor->Create(1);
                    print '</td></tr>';
                }
