...
{
	if (GETPOST('PROJECT_USE_OPPORTUNITIES')) mounir_set_const($db, "PROJECT_USE_OPPORTUNITIES", GETPOST('PROJECT_USE_OPPORTUNITIES'), 'chaine', 0, '', $conf->entity);
	else mounir_del_const($db, "PROJECT_USE_OPPORTUNITIES", $conf->entity);

	// Warning, the constant saved and used in code is PROJECT_HIDE_TASKS
	if (GETPOST('PROJECT_USE_TASKS')) mounir_del_const($db, "PROJECT_HIDE_TASKS", $conf->entity);
	else mounir_set_const($db, "PROJECT_HIDE_TASKS", 1, 'chaine', 0, '', $conf->entity);
}
...
elseif ($action == 'updateMask')
{
	...
	if ($maskconstproject)  $res = mounir_set_const($db, $maskconstproject, $maskproject, 'chaine', 0, '', $conf->entity);
...
if ($action == 'updateMaskTask')
{
	...
	if ($maskconstmasktask)  $res = mounir_set_const($db, $maskconstmasktask, $masktaskt, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
		if ($conf->global->PROJECT_ADDON_PDF == "$value") mounir_del_const($db, 'PROJECT_ADDON_PDF', $conf->entity);
	}
}
if ($action == 'deltask')
{
	...
	if ($ret > 0)
	{
		if ($conf->global->PROJECT_TASK_ADDON_PDF == "$value") mounir_del_const($db, 'PROJECT_TASK_ADDON_PDF', $conf->entity);
	}
}
...
{
	mounir_set_const($db, "PROJECT_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity);
...
{
	if (mounir_set_const($db, "PROJECT_TASK_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'setmod')
{
	...
	mounir_set_const($db, "PROJECT_ADDON", $value, 'chaine', 0, '', $conf->entity);
}
...
elseif ($action == 'setmodtask')
{
	...
	mounir_set_const($db, "PROJECT_TASK_ADDON", $value, 'chaine', 0, '', $conf->entity);
}
elseif ($action == 'updateoptions')
{
	if (GETPOST('PROJECT_USE_SEARCH_TO_SELECT'))
	{
		...
		if (mounir_set_const($db, "PROJECT_USE_SEARCH_TO_SELECT", $companysearch, 'chaine', 0, '', $conf->entity))
		{
	...
	if (GETPOST('PROJECT_ALLOW_TO_LINK_FROM_OTHER_COMPANY'))
	{
		...
		mounir_set_const($db, 'PROJECT_ALLOW_TO_LINK_FROM_OTHER_COMPANY', $projectToSelect, 'chaine', 0, '', $conf->entity);	//Allow to disable this configuration if empty value
	}
}
