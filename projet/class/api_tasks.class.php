...
class Tasks extends MounirApi
{
    ...
    {
		if(! MounirApiAccess::$user->rights->projet->lire) {
		...
		if( ! MounirApi::_checkAccessToResource('task', $this->task->id)) {
    ...
    {
        if (! MounirApiAccess::$user->rights->projet->creer) {
        ...
        if ($this->task->create(MounirApiAccess::$user) < 0) {
    ...
    public function getLines($id, $includetimespent=0)
    {
        ...
        if( ! MounirApi::_checkAccessToResource('project',$this->project->id)) {
            throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
        }
        $this->project->getLinesArray(MounirApiAccess::$user);
        $result = array();
    ...
    */
    ...
    public function getRoles($id, $userid = 0)
    {
        ...
        if( ! MounirApi::_checkAccessToResource('tasks', $this->task->id)) {
            throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
        }

        $usert = MounirApiAccess::$user;
        if ($userid > 0)
        {
    ...
    */
    ...
    public function put($id, $request_data = null)
    {
		...
		if ( ! MounirApi::_checkAccessToResource('tasks', $this->project->id)) {
			throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
		}
        foreach($request_data as $field => $value) {
        ...
        if ($this->task->update(MounirApiAccess::$user) > 0)
        {
            return $this->get($id);
        }
        else
    ...
    {
        if(! MounirApiAccess::$user->rights->projet->supprimer) {
            throw new RestException(401);
        }
        $result = $this->task->fetch($id);
        if( ! $result ) {
        ...
        if( ! $this->task->delete(MounirApiAccess::$user)) {
    ...
    public function addTimeSpent($id, $date, $duration, $user_id = 0, $note = '')
    {
        ...
        if( ! MounirApi::_checkAccessToResource('project', $this->task->fk_project)) {
            throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
        }

        $uid = $user_id;
        if (empty($uid)) $uid = MounirApiAccess::$user->id;
        ...
        $result = $this->task->addTimeSpent(MounirApiAccess::$user, 0);
        if ($result == 0) {
