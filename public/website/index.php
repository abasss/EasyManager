 ...
 *		\ingroup    website
 *		\brief      Wrapper to output pages when website is powered by ERP instead of a native web server
 */
...
if ($pageid == 'css')   // No more used ?
{
    ...
    // Important: Following code is to avoid page request by browser and PHP CPU at each ERP page access.
    //if (empty($mounir_nocache)) header('Cache-Control: max-age=3600, public, must-revalidate');
    //else
    header('Cache-Control: no-cache');
    $original_file=$mounir_main_data_root.'/website/'.$websitekey.'/styles.css.php';
}
else
{
    $original_file=$mounir_main_data_root.'/website/'.$websitekey.'/page'.$pageid.'.tpl.php';
}
...
print '<!-- Page content '.$original_file.' rendered with MOUNIR SERVER : Html with CSS link and html header + Body that was saved into tpl dir -->'."\n";
