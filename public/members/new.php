 ...
 *  MEMBER_NEWFORM_MOUNIRTURNOVER     Show field turnover (specific for mounir foundation)
...
if ($action == 'add')
{
    ...
    if (! empty($conf->global->MEMBER_NEWFORM_MOUNIRTURNOVER))
    {
    ...
    if (! $error)
    {
        ...
        if ($result > 0)
        {
            ...
            if ($object->email)
            {
            	...
            	{
            		$moreinheader='X-ERP-Info: send_an_email by public/members/new.php'."\r\n";
...
// Add specific fields used by ERP foundation for example
if (! empty($conf->global->MEMBER_NEWFORM_MOUNIRTURNOVER))
{
