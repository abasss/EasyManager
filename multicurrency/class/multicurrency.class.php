...
class MultiCurrency extends CommonObject
{
	 ...
	 */
    public function addRateFromMounir($code, $rate)
    {
        global $db, $user;
     ...
     * @param	string	$way			mounir mean the amount is in mounir currency
     * @param	string	$table			facture or facture_fourn
     * @return	double					amount converted
     */
    public static function getAmountConversionFromInvoiceRate($fk_facture, $amount, $way = 'mounir', $table = 'facture')
    {
        global $db;
        ...
        if ($multicurrency_tx)
        {
            if ($way == 'mounir') return $amount * $multicurrency_tx;
        ...
        if ($response->success)
        {
			...
			if (self::recalculRates($TRate) >= 0)
			{
				foreach ($TRate as $currency_code => $rate)
				{
					...
					{
						self::addRateFromMounir($code, $rate);
					}
				}
			}
		}
