 ...
 */
class Proposals extends MounirApi
{
    /**
	...
	public function get($id, $contact_list = 1)
	{
		...
		if( ! MounirApi::_checkAccessToResource('propal', $this->propal->id)) {
    ...
    {
        if(! MounirApiAccess::$user->rights->propal->creer) {
		...
		if ($this->propal->create(MounirApiAccess::$user) < 0) {
    ...
    public function put($id, $request_data = null)
    {
		...
		{
			if($this->propal->set_echeance(MounirApiAccess::$user, $this->propal->fin_validite)<0)
			{
				throw new RestException(500, $this->propal->error);
			}
		}

		if ($this->propal->update(MounirApiAccess::$user) > 0)
		{
			return $this->get($id);
		}
		else
	...
	{
		if(! MounirApiAccess::$user->rights->propal->supprimer) {
			throw new RestException(401);
		}
		$result = $this->propal->fetch($id);
		if( ! $result ) {
		...
		if( ! $this->propal->delete(MounirApiAccess::$user)) {
	...
	*/
	public function settodraft($id)
	{
		...
		$result = $this->propal->setDraft(MounirApiAccess::$user);
		if ($result == 0) {
    ...
    public function validate($id, $notrigger = 0)
    {
		...
		$result = $this->propal->valid(MounirApiAccess::$user, $notrigger);
		if ($result == 0) {
	...
	public function close($id, $status, $note_private = '', $notrigger = 0)
	{
		...
		$result = $this->propal->cloture(MounirApiAccess::$user, $status, $note_private, $notrigger);
		if ($result == 0) {
    ...
    public function setinvoiced($id)
    {
        ...
        $result = $this->propal->classifyBilled(MounirApiAccess::$user);
        if ($result < 0) {
            throw new RestException(500, 'Error : '.$this->propal->error);
        }
