...
if ($action == 'create')
{
	...
	//  MailErrorsTo -  MailTo            abass
	print '<tr><td class="fieldrequired">'.$langs->trans("MailTo").'</td><td><input class="flat minwidth200" name="errorsto" value="'.(!empty($conf->global->MAILING_EMAIL_ERRORSTO)?$conf->global->MAILING_EMAIL_ERRORSTO:$conf->global->MAIN_MAIL_ERRORS_TO).'"></td></tr>';
...
print '</br><br>';
	...
	$doleditor=new DolEditor('bodyemail', GETPOST('bodyemail', 'none'), '', 600, 'mounir_mailings', '', true, true, $conf->global->FCKEDITOR_ENABLE_MAILING, 20, '90%');
	$doleditor->Create();
	print '</div>';
	...
	if ($object->id > 0)
	{
		...
		if ($action != 'edit' && $action != 'edithtml')
		{
			...
			if (empty($object->bgcolor) || strtolower($object->bgcolor) == 'ffffff')	// CKEditor does not apply the color of the div into its content area
			{
				...
				// Editeur wysiwyg
				require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
				$doleditor=new DolEditor('bodyemail', $object->body, '', 600, 'mounir_mailings', '', false, true, empty($conf->global->FCKEDITOR_ENABLE_MAILING)?0:1, 20, '90%', $readonly);
				$doleditor->Create();
			}
		...
		else
		{
			...
			*/
			...
			if ($action == 'edit')
			{
				// Editeur wysiwyg
				require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
				$doleditor=new DolEditor('bodyemail', $object->body, '', 600, 'mounir_mailings', '', true, true, $conf->global->FCKEDITOR_ENABLE_MAILING, 20, '90%');
				$doleditor->Create();
			}
			if ($action == 'edithtml')
			{
				// Editor HTML source
				require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
				$doleditor=new DolEditor('bodyemail', $object->body, '', 600, 'mounir_mailings', '', true, true, 'ace', 20, '90%');
