 ...
 */
class AgendaEvents extends MounirApi
{
    ...
    {
        if (! MounirApiAccess::$user->rights->agenda->myactions->read) {
        ...
        if (! MounirApiAccess::$user->rights->agenda->allactions->read && $this->actioncomm->ownerid != MounirApiAccess::$user->id) {
            throw new RestException(401, "Insuffisant rights to read event for owner id ".$request_data['userownerid'].' Your id is '.MounirApiAccess::$user->id);
        }

        if ( ! MounirApi::_checkAccessToResource('agenda', $this->actioncomm->id, 'actioncomm', '', 'fk_soc', 'id')) {
    ...
    public function index($sortfield = "t.id", $sortorder = 'ASC', $limit = 100, $page = 0, $user_ids = 0, $sqlfilters = '')
    {
        ...
        $socid = 0;
        if (! empty(MounirApiAccess::$user->socid)) $socid = MounirApiAccess::$user->socid;

        // If the internal user must only see his customers, force searching by him
        $search_sale = 0;
        if (! MounirApiAccess::$user->rights->societe->client->voir && !$socid) $search_sale = MounirApiAccess::$user->id;
        ...
        if (! empty($conf->societe->enabled))
            if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socid) || $search_sale > 0) $sql .= ", sc.fk_soc, sc.fk_user"; // We need these fields in order to filter by sale (including the case where the user can only see his prospects)
        $sql.= " FROM ".MAIN_DB_PREFIX."actioncomm as t";
        if (! empty($conf->societe->enabled))
            if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socid) || $search_sale > 0) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc"; // We need this table joined to the select in order to filter by sale
        $sql.= ' WHERE t.entity IN ('.getEntity('agenda').')';
        if (! empty($conf->societe->enabled))
            if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socid) || $search_sale > 0) $sql.= " AND t.fk_soc = sc.fk_soc";
    ...
    {
        if (! MounirApiAccess::$user->rights->agenda->myactions->create) {
            throw new RestException(401, "Insuffisant rights to create your Agenda Event");
        }
        if (! MounirApiAccess::$user->rights->agenda->allactions->create && MounirApiAccess::$user->id != $request_data['userownerid']) {
            throw new RestException(401, "Insuffisant rights to create an Agenda Event for owner id ".$request_data['userownerid'].' Your id is '.MounirApiAccess::$user->id);
        }
        ...
        if ($this->actioncomm->create(MounirApiAccess::$user) < 0) {
    ...
    public function put($id, $request_data = null)
    {
        ...
        if ( ! MounirApi::_checkAccessToResource('actioncomm',$this->actioncomm->id)) {
            throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
        }
        foreach($request_data as $field => $value) {
        ...
        if ($this->actioncomm->update($id, MounirApiAccess::$user,1,'','','update'))
            return $this->get($id);
    ...
    */
    ...
    {
        if(! MounirApiAccess::$user->rights->agenda->myactions->delete) {
            throw new RestException(401, "Insuffisant rights to delete your Agenda Event");
        }

        $result = $this->actioncomm->fetch($id);

        if(! MounirApiAccess::$user->rights->agenda->allactions->delete && MounirApiAccess::$user->id != $this->actioncomm->userownerid) {
            throw new RestException(401, "Insuffisant rights to delete an Agenda Event of owner id ".$request_data['userownerid'].' Your id is '.MounirApiAccess::$user->id);
        }
        ...
        if( ! $this->actioncomm->delete(MounirApiAccess::$user)) {
