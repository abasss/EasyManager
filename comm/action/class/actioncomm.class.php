...
class ActionComm extends CommonObject
{
    ...
    public function build_exportfile($format, $type, $cachedelay, $filename, $filters)
    {
        ...
        // phpcs:enable
        global $conf,$langs,$mounir_main_url_root,$mysoc;
        ...
        if ($buildfile)
        {
            ...
            if ($resql)
            {
                ...
                while ($obj=$this->db->fetch_object($resql))
                {
                    ...
                    $event=array();
                    $event['uid']='mouniragenda-'.$this->db->database_name.'-'.$obj->id."@".$_SERVER["SERVER_NAME"];
                    $event['type']=$type;
            ...
            if ($more)
            {
                $title='ERP actions '.$mysoc->name.' - '.$more;
                $desc=$more;
                $desc.=' ('.$mysoc->name.' - built by ERP)';
            }
            else
            {
                $title='ERP actions '.$mysoc->name;
