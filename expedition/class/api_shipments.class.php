 ...
 */
class Shipments extends MounirApi
{
    ...
    {
        if(! MounirApiAccess::$user->rights->expedition->lire) {
        ...
        if( ! MounirApi::_checkAccessToResource('expedition', $this->shipment->id)) {
    ...
    {
        if (! MounirApiAccess::$user->rights->expedition->creer) {
        ...
        if ($this->shipment->create(MounirApiAccess::$user) < 0) {
    ...
    */
    ...
    public function deleteLine($id, $lineid)
    {
        ...
        $request_data = (object) $request_data;
        $updateRes = $this->shipment->deleteline(MounirApiAccess::$user, $lineid);
        if ($updateRes > 0) {
            return $this->get($id);
        }
        else
    ...
    public function put($id, $request_data = null)
    {
        ...
        if ($this->shipment->update(MounirApiAccess::$user) > 0)
        {
            return $this->get($id);
        }
        else
    ...
    {
        if(! MounirApiAccess::$user->rights->shipment->supprimer) {
            throw new RestException(401);
        }
        $result = $this->shipment->fetch($id);
        if( ! $result ) {
        ...
        if( ! $this->shipment->delete(MounirApiAccess::$user)) {
    ...
    public function validate($id, $notrigger = 0)
    {
        ...
        $result = $this->shipment->valid(MounirApiAccess::$user, $notrigger);
        if ($result == 0) {
    ...
    public function setinvoiced($id)
    {
        ...
        $result = $this->shipment->classifyBilled(MounirApiAccess::$user);
        if( $result < 0) {
    ...
    */
    ...
    public function createShipmentFromOrder($orderid)
    {
        ...
        $result = $this->shipment->createFromOrder($order, MounirApiAccess::$user);
        if( $result < 0) {
                throw new RestException(405, $this->shipment->error);
        }
        $this->shipment->fetchObjectLinked();
