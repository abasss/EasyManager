...
$pathofwebsite=$mounir_main_data_root.'/website/'.$websitekey;
...
{
	mounir_set_const($db, 'WEBSITE_EDITINLINE', 1);
	setEventMessages($langs->trans("FeatureNotYetAvailable"), null, 'warnings');
	mounir_set_const($db, 'WEBSITE_SUBCONTAINERSINLINE', 0);	// Force disable of 'Include dynamic content'
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
if ($action == 'unseteditinline')
{
	mounir_del_const($db, 'WEBSITE_EDITINLINE');
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
if ($action == 'setshowsubcontainers')
{
	mounir_set_const($db, 'WEBSITE_SUBCONTAINERSINLINE', 1);
	mounir_set_const($db, 'WEBSITE_EDITINLINE', 0);	// Force disable of edit inline
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
if ($action == 'unsetshowsubcontainers')
{
	mounir_del_const($db, 'WEBSITE_SUBCONTAINERSINLINE');
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
...
if ($action == 'addcontainer')
{
	...
	if ($urltograb)
	{
		...
		if (! $error)
		{
			...
			else
			{
				...
				// Normalize links href to ERP internal naming
	...
	if (! $error)
	{
		...
		{
			$robotcontent = "# Robot file. Generated with ERP\nUser-agent: *\nAllow: /public/\nDisallow: /administrator/";
...
if ($action == 'updatecss')
{
	...
	else
	{
		...
		if (! $error)
		{
    	    ...
    	    $htmlheadercontent.= "if (! defined('USEMOUNIRSERVER') && ! defined('USEMOUNIREDITOR')) { require_once './master.inc.php'; } // Not already loaded"."\n";
    		...
    		$csscontent.= "if (! defined('USEMOUNIRSERVER') && ! defined('USEMOUNIREDITOR')) { require_once __DIR__.'/master.inc.php'; } // Not already loaded"."\n";	// For the css, we need to set path of master using the dirname of css file.
    		...
    		$jscontent.= "if (! defined('USEMOUNIRSERVER') && ! defined('USEMOUNIREDITOR')) { require_once __DIR__.'/master.inc.php'; } // Not already loaded"."\n";	// For the css, we need to set path of master using the dirname of css file.
    	    ...
    	    $robotcontent.= "if (! defined('USEMOUNIRSERVER') && ! defined('USEMOUNIREDITOR')) { require_once './master.inc.php'; } // Not already loaded"."\n";
        	...
        	$htaccesscontent.= "if (! defined('USEMOUNIRSERVER') && ! defined('USEMOUNIREDITOR')) { require_once './master.inc.php'; } // Not already loaded"."\n";
	...
	if ($action == 'confirm_createpagefromclone')
	{
		...
		if (! $error)
		{
			...
			if ($newwebsiteid > 0 && $newwebsiteid != $object->id)
			{
				...
				$pathofwebsitenew = $mounir_main_data_root.'/website/'.$tmpwebsite->ref;
			}
			else
			{
				$tmpwebsite = $object;
...
if (! GETPOST('hide_websitemenu'))
{
	...
	if ($websitekey && $websitekey != '-1' && ! in_array($action, array('editcss','editmenu','importsite')))
	{
		...
		if (($pageid > 0 && $atleastonepage) && ($action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone' || $action == 'deletesite'))
		{
			...
			$htmltext = $langs->trans("PreviewSiteServedByMounir", $langs->transnoentitiesnoconv("Page"), $langs->transnoentitiesnoconv("Page"), $realpage, $dataroot);
			$htmltext.='<br>'.$langs->trans("CheckVirtualHostPerms", $langs->transnoentitiesnoconv("ReadPerm"), DOL_DOCUMENT_ROOT);
			$htmltext.='<br>'.$langs->trans("CheckVirtualHostPerms", $langs->transnoentitiesnoconv("WritePerm"), DOL_DATA_ROOT.'/website<br>'.DOL_DATA_ROOT.'/medias');
...
if ($action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone')
{
	if ($pageid > 0 && $atleastonepage)
	{
		...
		// Ouput page under the ERP top menu
		...
		*/
		...
		// Clean style that may affect global style of ERP
