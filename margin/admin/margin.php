...
{
    $code=$reg[1];
    if (mounir_set_const($db, $code, 1, 'yesno', 0, '', $conf->entity) > 0)
    {
...
if ($action == 'remises')
{
    if (mounir_set_const($db, 'MARGIN_METHODE_FOR_DISCOUNT', $_POST['MARGIN_METHODE_FOR_DISCOUNT'], 'chaine', 0, '', $conf->entity) > 0)
    {
...
{
    if (mounir_set_const($db, 'MARGIN_TYPE', $_POST['MARGIN_TYPE'], 'chaine', 0, '', $conf->entity) > 0)
    {
          setEventMessages($langs->trans("RecordModifiedSuccessfully"), null, 'mesgs');
    }
    else
...
if ($action == 'contact')
{
    if (mounir_set_const($db, 'AGENT_CONTACT_TYPE', $_POST['AGENT_CONTACT_TYPE'], 'chaine', 0, '', $conf->entity) > 0)
    {
          setEventMessages($langs->trans("RecordModifiedSuccessfully"), null, 'mesgs');
    }
    else
