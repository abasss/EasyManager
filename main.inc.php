...
// This is to make ERP working with Plesk
...
// Init session. Name of session is specific to ERP instance.
...
// Force HTTPS if required ($conf->file->main_force_https is 0/1 or https mounir root url)
...
if (! empty($conf->file->main_force_https) && (empty($_SERVER["HTTPS"]) || $_SERVER["HTTPS"] != 'on'))
{
	...
	if ($newurl)
	{
		dol_syslog("main.inc: mounir_main_force_https is on, we make a redirect to ".$newurl);
		header("Location: ".$newurl);
		exit;
	}
	else
	{
		dol_syslog("main.inc: mounir_main_force_https is on but we failed to forge new https url so no redirect is done", LOG_WARNING);
	}
}

if (! defined('NOLOGIN') && ! defined('NOIPCHECK') && ! empty($mounir_main_restrict_ip))
{
	$listofip=explode(',', $mounir_main_restrict_ip);
	$found = false;
...
|| (empty($conf->global->MAIN_VERSION_LAST_UPGRADE) && ! empty($conf->global->MAIN_VERSION_LAST_INSTALL) && ($conf->global->MAIN_VERSION_LAST_INSTALL != DOL_VERSION)))
{
	...
	$mounirversionlastupgrade=preg_split('/[.-]/', $versiontocompare);
	$mounirversionprogram=preg_split('/[.-]/', DOL_VERSION);
	$rescomp=versioncompare($mounirversionprogram, $mounirversionlastupgrade);
...
//var_dump(GETPOST('token').' '.$_SESSION['token'].' - '.$_SESSION['newtoken'].' '.$_SERVER['SCRIPT_FILENAME']);
//$mounir_nocsrfcheck=1;
// Check token
//var_dump((! defined('NOCSRFCHECK')).' '.empty($mounir_nocsrfcheck).' '.(! empty($conf->global->MAIN_SECURITY_CSRF_WITH_TOKEN)).' '.$_SERVER['REQUEST_METHOD'].' '.(! GETPOSTISSET('token')));
if ((! defined('NOCSRFCHECK') && empty($mounir_nocsrfcheck) && ! empty($conf->global->MAIN_SECURITY_CSRF_WITH_TOKEN))
	|| defined('CSRFCHECK_WITH_TOKEN'))	// Check validity of token, only if option MAIN_SECURITY_CSRF_WITH_TOKEN enabled or if constant CSRFCHECK_WITH_TOKEN is set
{
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && ! GETPOSTISSET('token')) // Note, offender can still send request by GET
	{
		...
		print "If you access your server behind a proxy using url rewriting, you might check that all HTTP header is propagated (or add the line \$mounir_nocsrfcheck=1 into your conf.php file or MAIN_SECURITY_CSRF_WITH_TOKEN to 0 into setup).\n";
		die;
	}
...
if (! defined('NOLOGIN'))
{
	...
	// Example: 'http', 'mounir', 'ldap', 'http,forceuser', '...'

	if (defined('MAIN_AUTHENTICATION_MODE'))
	{
		$mounir_main_authentication = constant('MAIN_AUTHENTICATION_MODE');
	}
	else
	{
		// Authentication mode
	...
	{
		$langs->load('main');
		dol_print_error('', $langs->trans("ErrorConfigParameterNotDefined", 'mounir_main_authentication'));
		exit;
	}
	...
	if (! isset($_SESSION["dol_login"]))
	{
		...
		if (! empty($mounir_main_demo) && $_SERVER['PHP_SELF'] == DOL_URL_ROOT.'/index.php')  // We ask index page
		{
		...
		$usertotest		= (! empty($_COOKIE['login_mounir']) ? $_COOKIE['login_mounir'] : GETPOST("username", "alpha", $allowedmethodtopostusername));
		...
		if ($mounir_main_authentication == 'forceuser' && ! empty($mounir_auto_user)) $goontestloop=true;
		if (GETPOST("username", "alpha", $allowedmethodtopostusername) || ! empty($_COOKIE['login_mounir']) || GETPOST('openid_mode', 'alpha', 1)) $goontestloop=true;
		...
		if ($resultFetchUser <= 0)
		{
			...
			if ($resultFetchUser == 0)
			{
				...
				$_SESSION["dol_loginmesg"]=$langs->trans("ErrorCantLoadUserFromMounirDatabase", $login);

				$user->trigger_mesg='ErrorCantLoadUserFromMounirDatabase - login='.$login;
			}
			if ($resultFetchUser < 0)
			{
...
function top_htmlhead($head, $title = '', $disablejs = 0, $disablehead = 0, $arrayofjs = '', $arrayofcss = '', $disablejmobile = 0, $disablenofollow = 0)
{
	...
	if (empty($disablehead))
	{
		...
		print '<meta name="author" content="ERP Development Team">'."\n";
		...
		//if (empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) print '<link rel="author" title="ERP Development Team" href="https://www.google.com">'."\n";
		...
		print '<!-- Includes CSS for ERP theme -->'."\n";
        ...
        if (! $disablejs && ! empty($conf->use_javascript_ajax))
        {
            ...
            if (! defined('DISABLE_BROWSER_NOTIF'))
            {
                ...
                {
                    print '<!-- Includes JS of ERP (brwoser layout = '.$conf->browser->layout.')-->'."\n";
                    print '<script src="'.DOL_URL_ROOT.'/core/js/lib_notification.js.php'.($ext?'?'.$ext:'').'"></script>'."\n";
                }
            }

            // Global js function
            print '<!-- Includes JS of ERP -->'."\n";
...
function top_menu($head, $title = '', $target = '', $disablejs = 0, $disablehead = 0, $arrayofjs = '', $arrayofcss = '', $morequerystring = '', $helppagename = '')
{
	...
	global $mounir_main_authentication, $mounir_main_demo;
...
function top_menu_user(User $user, Translate $langs)
{
    ...
    $dropdownBody.= '<br><b>'.$langs->trans("AuthenticationMode").':</b> '.$_SESSION["dol_authmode"].(empty($mounir_main_demo)?'':' (demo)');
...
function left_menu($menu_array_before, $helppagename = '', $notused = '', $menu_array_after = '', $leftmenuwithoutmainarea = 0, $title = '', $acceptdelayedhtml = 0)
{
	...
	if (empty($conf->dol_hide_leftmenu) && (! defined('NOREQUIREMENU') || ! constant('NOREQUIREMENU')))
	{
		...
		// ERP version + help + bug report link
		print "\n";
		...
		{
			$doliurl='https://www.google.com';
			//local communities
			if (preg_match('/fr/i', $langs->defaultlang)) $doliurl='https://www.google.fr';
			if (preg_match('/es/i', $langs->defaultlang)) $doliurl='https://www.google.es';
			if (preg_match('/de/i', $langs->defaultlang)) $doliurl='https://www.google.de';
			if (preg_match('/it/i', $langs->defaultlang)) $doliurl='https://www.google.it';
			if (preg_match('/gr/i', $langs->defaultlang)) $doliurl='https://www.google.gr';
		...
		if (! empty($conf->global->MAIN_BUGTRACK_ENABLELINK))
		{
			...
			$bugbaseurl = 'https://github.com/ERP/mounir/issues/new';
			$bugbaseurl.= '?title=';
...
function getHelpParamFor($helppagename, $langs)
{
	...
	else
	{
		...
		{
			$helpbaseurl='http://www.google.com/index.php/%s';
