...
$result	= $object->updateline(
	...
	if ($action == 'webservice' && GETPOST('mode', 'alpha') == "send" && ! GETPOST('cancel', 'alpha'))
	{
		...
		$ws_ns='http://www.google.com/ns/';
		$ws_authentication=array(
			'mounirkey'=>$ws_key,
			'sourceapplication'=>'MounirWebServiceClient',
			'login'=>$ws_user,
...
if ($action=='create')
{
	...
	print '<td>';
	$doleditor = new DolEditor('note_public', isset($note_public) ? $note_public : GETPOST('note_public', 'none'), '', 80, 'mounir_notes', 'In', 0, false, true, ROWS_3, '90%');
	...
	print '<td>';
	$doleditor = new DolEditor('note_private', isset($note_private) ? $note_private : GETPOST('note_private', 'none'), '', 80, 'mounir_notes', 'In', 0, false, true, ROWS_3, '90%');
	print $doleditor->Create(1);
	print '</td>';
				...
				if ($user_status_code == "OK")
				{
					...
					else
					{
						...
						foreach ($object->lines as $line) {
							...
							$supplier_price = price($result_product["product"]["price_net"]); //Price of client tab in supplier mounir
							$local_price = null; //Price of supplier as stated in product suppliers tab on this mounir, NULL if not found
