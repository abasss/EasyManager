 ...
 */
class SupplierOrders extends MounirApi
{
    /**
     *
    ...
    {
        if(! MounirApiAccess::$user->rights->fournisseur->commande->lire) {
        ...
        if ( ! MounirApi::_checkAccessToResource('fournisseur', $this->order->id, '', 'commande')) {
    ...
    {
        if(! MounirApiAccess::$user->rights->fournisseur->commande->creer) {
        ...
        if ($this->order->create(MounirApiAccess::$user) < 0) {
    ...
    public function put($id, $request_data = null)
    {
        ...
        if($this->order->update($id, MounirApiAccess::$user))
            return $this->get($id);
    ...
    {
        if (! MounirApiAccess::$user->rights->fournisseur->commande->supprimer) {
            throw new RestException(401);
        }
        $result = $this->order->fetch($id);
        if ( ! $result) {
        ...
        if ( $this->order->delete(MounirApiAccess::$user) < 0) {
    ...
    public function validate($id, $idwarehouse = 0, $notrigger = 0)
    {
        ...
        $result = $this->order->valid(MounirApiAccess::$user, $idwarehouse, $notrigger);
        if ($result == 0) {
