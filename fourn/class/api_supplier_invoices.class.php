 ...
 */
class SupplierInvoices extends MounirApi
{
    /**
     *
    ...
    {
        if(! MounirApiAccess::$user->rights->fournisseur->facture->lire) {
        ...
        if( ! MounirApi::_checkAccessToResource('fournisseur', $this->invoice->id, 'facture_fourn', 'facture')) {
    ...
    {
        if(! MounirApiAccess::$user->rights->fournisseur->facture->creer) {
        ...
        if ($this->invoice->create(MounirApiAccess::$user) < 0) {
    ...
    public function put($id, $request_data = null)
    {
        ...
        if($this->invoice->update($id, MounirApiAccess::$user))
            return $this->get($id);
    ...
    {
        if(! MounirApiAccess::$user->rights->fournisseur->facture->supprimer) {
            throw new RestException(401);
        }
        $result = $this->invoice->fetch($id);
        if( ! $result ) {
        ...
        if( $this->invoice->delete(MounirApiAccess::$user) < 0)
        {
