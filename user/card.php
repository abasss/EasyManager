...
if ($action == 'create' || $action == 'adduserldap')
{
	...
	if (! empty($conf->ldap->enabled) && (isset($conf->global->LDAP_SYNCHRO_ACTIVE) && $conf->global->LDAP_SYNCHRO_ACTIVE == 'ldap2mounir'))
	{
		/*
         * Affiche formulaire d'ajout d'un compte depuis LDAP
         * si on est en synchro LDAP vers ERP
         */
	...
	print '<td>';
	$valuetoshow='';
	if (preg_match('/ldap/', $mounir_main_authentication))
	{
		$valuetoshow.=($valuetoshow?', ':'').$langs->trans("PasswordOfUserInLDAP");
	}
	if (preg_match('/http/', $mounir_main_authentication))
	{
		$valuetoshow.=($valuetoshow?', ':'').$langs->trans("HTTPBasicPassword");
	}
	if (preg_match('/mounir/', $mounir_main_authentication))
	{
		if (! empty($ldap_pass))	// For very old system comaptibilty. Now clear password can't be viewed from LDAP read
		{
			$valuetoshow.= ($valuetoshow?', ':'').'<input type="hidden" name="password" value="'.$ldap_pass.'">';	// ERP password is preffiled with LDAP known password
...
*/
	...
	$doleditor=new DolEditor('note', '', '', 120, 'mounir_notes', '', false, true, $conf->global->FCKEDITOR_ENABLE_SOCIETE, ROWS_3, '90%');
	$doleditor->Create();
	print "</td></tr>\n";
	...
	// print '<td>';
	// require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
	// $doleditor=new DolEditor('signature', GETPOST('signature'), '', 138, 'mounir_notes', 'In', true, true, empty($conf->global->FCKEDITOR_ENABLE_USERSIGN)?0:1, ROWS_4, '90%');
...
else
{
	...
	if ($id > 0)
	{
		...
		if ($action != 'edit')
		{
			...
			{
				print '<td class="error">'.$langs->trans("LoginAccountDisableInMounir").'</td>';
			}
			else
			{
			...
			{
				$langs->load("members");
				print '<tr><td>'.$langs->trans("LinkedToMounirMember").'</td>';
				print '<td>';
				if ($object->fk_member)
				{
					$adh=new Adherent($db);
		...
		if ($action == 'edit' && ($canedituser || $caneditfield || $caneditpassword || ($user->id == $object->id)))
		{
			...
			if (preg_match('/http/', $mounir_main_authentication))
			{
				$valuetoshow.=($valuetoshow?(' '.$langs->trans("or").' '):'').$form->textwithpicto($text, $langs->trans("MounirInHttpAuthenticationSoPasswordUseless", $mounir_main_authentication), 1, 'warning');
			}
			if (preg_match('/mounir/', $mounir_main_authentication))
			{
				if ($caneditpassword)
...
*/
			...
			{
				$langs->load("members");
				print '<tr><td width="25%">'.$langs->trans("LinkedToMounirMember").'</td>';
				print '<td>';
				if ($object->fk_member)
				{
					$adh=new Adherent($db);
			...
			// {
				// require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
				// $doleditor=new DolEditor('signature', $object->signature, '', 138, 'mounir_notes', 'In', false, true, empty($conf->global->FCKEDITOR_ENABLE_USERSIGN)?0:1, ROWS_4, '90%');
				// print $doleditor->Create(1);
			// }
			// else
			// {
