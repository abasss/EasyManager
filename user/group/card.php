...
if ($action == 'create')
{
    ...
    $doleditor=new DolEditor('note', '', '', 240, 'mounir_notes', '', false, true, $conf->global->FCKEDITOR_ENABLE_SOCIETE, ROWS_8, '90%');
    $doleditor->Create();
    print "</td></tr>\n";
...
else
{
    if ($id)
    {
        ...
        if ($action == 'edit' && $caneditperms)
        {
            ...
            require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
            $doleditor=new DolEditor('note', $object->note, '', 240, 'mounir_notes', '', true, false, $conf->global->FCKEDITOR_ENABLE_SOCIETE, ROWS_8, '90%');
            $doleditor->Create();
            print '</td>';
            print "</tr>\n";
			// Other attributes
