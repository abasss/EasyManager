...
if ($action == 'edit')
{
    ...
    print empty($mounir_main_demo)?'':' disabled="disabled"';	// Disabled for demo
    ...
    print '> '.$langs->trans("UsePersonalValue").'</td>';
    print '<td>';
    print $formadmin->select_language((! empty($object->conf->MAIN_LANG_DEFAULT)?$object->conf->MAIN_LANG_DEFAULT:''), 'main_lang_default', 1, null, 0, 0, (! empty($mounir_main_demo)));
    print '</td></tr>';
    ...
    // Theme
    show_theme($object, (($user->admin || empty($mounir_main_demo))?1:0), true);
...
else
{
    ...
    if (empty($user->admin) && ! empty($mounir_main_demo))
    {
