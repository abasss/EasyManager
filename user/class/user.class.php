...
class User extends CommonObject
{
	...
	public function getNomUrl($withpictoimg = 0, $option = '', $infologin = 0, $notooltip = 0, $maxlen = 24, $hidethirdpartylogo = 0, $mode = '', $morecss = '', $save_lastsearch_value = -1)
	{
		...
		if ($infologin > 0)
		{
			...
			$label.= '<br><b>'.$langs->trans("AuthenticationMode").':</b> '.$_SESSION["dol_authmode"].(empty($mounir_main_demo)?'':' (demo)');
	 ...
	 *  @return int  				<0 if KO, >0 if OK
	 */
	public function update_ldap2mounir(&$ldapuser)
	{
        // phpcs:enable
		...
		dol_syslog(get_class($this)."::update_ldap2mounir result=".$result, LOG_DEBUG);
