...
class Users extends MounirApi
{
    ...
    public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $user_ids = 0, $sqlfilters = '')
    {
		...
		if(! MounirApiAccess::$user->rights->user->user->lire) {
	       throw new RestException(401, "You are not allowed to read list of users");
	    }

	    // case of external user, $societe param is ignored and replaced by user's socid
	    //$socid = MounirApiAccess::$user->societe_id ? MounirApiAccess::$user->societe_id : $societe;
    ...
    public function get($id)
    {
		...
		if (!MounirApi::_checkAccessToResource('user', $this->useraccount->id, 'user'))
		{
    ...
    public function post($request_data = null)
    {
	    ...
	    //if(! MounirApiAccess::$user->rights->user->creer) {
	    ...
	    if ($this->useraccount->create(MounirApiAccess::$user) < 0) {
    ...
    public function put($id, $request_data = null)
    {
		...
		if ($this->useraccount->update(MounirApiAccess::$user) >= 0)
		{
			return $this->get($id);
		}
		else
    ...
    public function setGroup($id, $group, $entity = 1)
    {
		...
		//if (!MounirApiAccess::$user->rights->user->user->supprimer) {
			//throw new RestException(401);
		//}
		$result = $this->useraccount->fetch($id);
		if (!$result)
		...
		if (! empty($conf->multicompany->enabled) && ! empty($conf->global->MULTICOMPANY_TRANSVERSE_MODE) && ! empty(MounirApiAccess::$user->admin) && empty(MounirApiAccess::$user->entity))
		{
		...
		else
		{
			...
			$entity = (MounirApiAccess::$user->entity > 0 ? MounirApiAccess::$user->entity : $conf->entity);
		}
    ...
    public function delete($id)
    {
		...
		return $this->useraccount->delete(MounirApiAccess::$user);
	}
	...
	protected function _cleanObjectDatas($object)
	{
	    ...
	    $canreadsalary = ((! empty($conf->salaries->enabled) && ! empty(MounirApiAccess::$user->rights->salaries->read))
	    	|| (! empty($conf->hrm->enabled) && ! empty(MounirApiAccess::$user->rights->hrm->employee->read)));
