...
/**
 *		\file 		htdocs/admin/tools/mounir_import.php
 *		\ingroup	core
...
<?php
print $langs->trans("RestoreDesc3", $mounir_main_db_name).'<br><br>';
?>

<?php print $langs->trans("DatabaseName").' : <b>'.$mounir_main_db_name.'</b>'; ?><br><br>
...
?>
	...
	$param=$mounir_main_db_name;
	$param.=" -h ".$mounir_main_db_host;
	...
	if (! empty($mounir_main_db_pass))
	{
		...
		$paramclear.=" -p".$mounir_main_db_pass;
	}
	...
	if (empty($_GET["showpass"]) && $mounir_main_db_pass) print '<br><a href="'.$_SERVER["PHP_SELF"].'?showpass=1&amp;radio_dump=mysql_options">'.$langs->trans("UnHidePassword").'</a>';
...
?>
    ...
    print ajax_autoselect('restorecommand');
    //if (empty($_GET["showpass"]) && $mounir_main_db_pass) print '<br><a href="'.$_SERVER["PHP_SELF"].'?showpass=1&amp;radio_dump=postgresql_options">'.$langs->trans("UnHidePassword").'</a>';
    //else print '<br><a href="'.$_SERVER["PHP_SELF"].'?showpass=0&amp;radio_dump=mysql_options">'.$langs->trans("HidePassword").'</a>';
    ?>
    </div>
    </fieldset>
