...
if ($file && ! $what)
{
    //print DOL_URL_ROOT.'/mounir_export.php';
	header("Location: ".DOL_URL_ROOT.'/admin/tools/mounir_export.php?msg='.urlencode($langs->trans("ErrorFieldRequired", $langs->transnoentities("ExportMethod"))).(GETPOST('page_y', 'int')?'&page_y='.GETPOST('page_y', 'int'):''));
    exit;
}
...
if ($what == 'mysql')
{
    ...
    if (! empty($mounir_main_restrict_os_commands))
    {
        $arrayofallowedcommand=explode(',', $mounir_main_restrict_os_commands);
        dol_syslog("Command are restricted to ".$mounir_main_restrict_os_commands.". We check that one of this command is inside ".$cmddump);
    ...
    if (! $errormsg && $cmddump)
    {
        mounir_set_const($db, 'SYSTEMTOOLS_MYSQLDUMP', $cmddump, 'chaine', 0, '', $conf->entity);
    }
...
if ($what == 'postgresql')
{
    ...
    if (! $errormsg && $cmddump)
    {
        mounir_set_const($db, 'SYSTEMTOOLS_POSTGRESQLDUMP', $cmddump, 'chaine', 0, '', $conf->entity);
    }
...
else
{
	...
	/*else
	{
		setEventMessages($langs->trans("YouMustRunCommandFromCommandLineAfterLoginToUser",$mounir_main_db_user,$mounir_main_db_user), null, 'warnings');
	}*/
}
...
$result=$formfile->list_of_documents($filearray,null,'systemtools','',1,'backup/',1,0,($langs->trans("NoBackupFileAvailable").'<br>'.$langs->trans("ToBuildBackupFileClickHere",DOL_URL_ROOT.'/admin/tools/mounir_export.php')),0,$langs->trans("PreviousDumpFiles"));

print '<br>';
*/

// Redirect to backup page
header("Location: mounir_export.php".(GETPOST('page_y', 'int')?'?page_y='.GETPOST('page_y', 'int'):''));
