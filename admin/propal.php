...
if ($action == 'updateMask')
{
	...
	if ($maskconstpropal) $res = mounir_set_const($db, $maskconstpropal, $maskpropal, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'set_PROPALE_DRAFT_WATERMARK')
{
	...
	$res = mounir_set_const($db, "PROPALE_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	if (! $res > 0) $error++;
...
elseif ($action == 'set_PROPOSAL_FREE_TEXT')
{
	...
	$res = mounir_set_const($db, "PROPOSAL_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
...
{
	$res = mounir_set_const($db, "PROPALE_VALIDITY_DURATION", $value, 'chaine', 0, '', $conf->entity);
...
{
    $res = mounir_set_const($db, "BANK_ASK_PAYMENT_BANK_DURING_PROPOSAL", $value, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->PROPALE_ADDON_PDF == "$value") mounir_del_const($db, 'PROPALE_ADDON_PDF', $conf->entity);
	}
}

elseif ($action == 'setdoc')
{
    if (mounir_set_const($db, "PROPALE_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'setmod')
{
	...
	mounir_set_const($db, "PROPALE_ADDON", $value, 'chaine', 0, '', $conf->entity);
}
