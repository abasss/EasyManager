...
if ($action == 'updateMask')
{
    ...
    if ($maskconstorder)  $res = mounir_set_const($db, $maskconstorder, $maskvalue, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->COMMANDE_SUPPLIER_ADDON_PDF == "$value") mounir_del_const($db, 'COMMANDE_SUPPLIER_ADDON_PDF', $conf->entity);
	}
}
...
{
	if (mounir_set_const($db, "COMMANDE_SUPPLIER_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'setmod')
{
    ...
    mounir_set_const($db, "COMMANDE_SUPPLIER_ADDON_NUMBER", $value, 'chaine', 0, '', $conf->entity);
}
...
elseif ($action == 'set_SUPPLIER_ORDER_OTHER')
{
    ...
    $res1 = mounir_set_const($db, "SUPPLIER_ORDER_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
    $res2 = mounir_set_const($db, "SUPPLIER_ORDER_3_STEPS_TO_BE_APPROVED", $doubleapproval, 'chaine', 0, '', $conf->entity);
...
{
    $res = mounir_set_const($db, "BANK_ASK_PAYMENT_BANK_DURING_SUPPLIER_ORDER", $value, 'chaine', 0, '', $conf->entity);
