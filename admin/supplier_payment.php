...
if ($action == 'updateMask')
{
    ...
    if ($maskconstsupplierpayment) $res = mounir_set_const($db, $maskconstsupplierpayment, $masksupplierpayment, 'chaine', 0, '', $conf->entity);
...
{
    mounir_set_const($db, "SUPPLIER_PAYMENT_ADDON", $value, 'chaine', 0, '', $conf->entity);
}
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->FACTURE_ADDON_PDF == "$value") mounir_del_const($db, 'SUPPLIER_PAYMENT_ADDON_PDF', $conf->entity);
	}
}
...
{
	if (mounir_set_const($db, "SUPPLIER_PAYMENT_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
