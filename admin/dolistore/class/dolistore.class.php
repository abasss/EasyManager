...
class Dolistore
{
	...
	public function getRemoteProducts($options = array('start' => 0, 'end' => 10, 'per_page' => 50, 'categorie' => 0, 'search' => ''))
	{
		...
		try {
			...
			$opt['display']        = '[id,name,id_default_image,id_category_default,reference,price,condition,show_price,date_add,date_upd,description_short,description,module_version,mounir_min,mounir_max]';
			$opt['sort']           = 'id_desc';
			$opt['filter[active]'] = '[1]';
	...
	public function get_products()
	{
		...
		foreach ($this->products as $product) {
			...
			//checking versions
			if ($this->version_compare($product->mounir_min, DOL_VERSION) <= 0) {
				if ($this->version_compare($product->mounir_max, DOL_VERSION) >= 0) {
					//compatible
                    $version = '<span class="compatible">'.$langs->trans('CompatibleUpTo', $product->mounir_max,
						$product->mounir_min, $product->mounir_max).'</span>';
					$compatible = '';
				} else {
