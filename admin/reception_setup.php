...
if (! empty($conf->reception->enabled) && empty($conf->global->MAIN_SUBMODULE_RECEPTION))
{
    ...
    mounir_set_const($db, "MAIN_SUBMODULE_RECEPTION", "1", 'chaine', 0, '', $conf->entity);
}
...
if ($action == 'updateMask')
{
	...
	if (! empty($maskconst))
		$res = mounir_set_const($db, $maskconst, $maskvalue, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'set_param')
{
	...
	$res = mounir_set_const($db, "RECEPTION_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
	if ($res <= 0)
	{
		$error++;
		setEventMessages($langs->trans("Error"), null, 'errors');
	...
	$res = mounir_set_const($db, "RECEPTION_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	if ($res <= 0)
	{
		$error++;
		setEventMessages($langs->trans("Error"), null, 'errors');
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->RECEPTION_ADDON_PDF == "$value") mounir_del_const($db, 'RECEPTION_ADDON_PDF', $conf->entity);
	}
}
...
{
	if (mounir_set_const($db, "RECEPTION_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
{
	mounir_set_const($db, "RECEPTION_ADDON_NUMBER", $value, 'chaine', 0, '', $conf->entity);
}
