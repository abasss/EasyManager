...
// We save list of template email ERP can manage. This list can found by a grep into code on "->param['models']"
$elementList = array();
	...
	else
	{
		if ($context != 'hide') {
			...
			if (empty($conf->global->FCKEDITOR_ENABLE_MAIL))
				$okforextended = false;
			$doleditor = new DolEditor($tmpfieldlist, (! empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 120, 'mounir_mailings', 'In', 0, false, $okforextended, ROWS_4, '90%');
                ...
                foreach ($fieldsforcontent as $tmpfieldlist)
                {
                    ...
                    if ($showfield) {
                        ...
                        if ($tmpfieldlist == 'content')
                        {
                        	...
                        	$doleditor = new DolEditor($tmpfieldlist.'-'.$rowid, (! empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 140, 'mounir_mailings', 'In', 0, false, $okforextended, ROWS_6, '90%');
                        	print $doleditor->Create(1);
                        }
                        print '</td>';
                        print '<td></td>';
            ...
            else
            {
                ...
                foreach ($fieldsforcontent as $tmpfieldlist)
                {
                    ...
                    if ($showfield) {
                        ...
                        if (empty($conf->global->FCKEDITOR_ENABLE_MAIL))
                            $okforextended = false;
                        $doleditor = new DolEditor($tmpfieldlist.'-'.$i, (! empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 140, 'mounir_mailings', 'In', 0, false, $okforextended, ROWS_6, '90%', 1);
                        print $doleditor->Create(1);
                        print '</td>';
                        print '<td></td><td></td><td></td>';
