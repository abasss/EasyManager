...
$mode=GETPOST('mode')?GETPOST('mode', 'alpha'):'mounir_notes';
...
foreach($modules as $const => $desc)
{
    ...
    {
        mounir_set_const($db, "FCKEDITOR_ENABLE_".$const, "1", 'chaine', 0, '', $conf->entity);
        ...
        {
            mounir_set_const($db, "FCKEDITOR_ENABLE_DETAILS", "1", 'chaine', 0, '', $conf->entity);
        }
    ...
    {
        mounir_del_const($db, "FCKEDITOR_ENABLE_".$const, $conf->entity);
        header("Location: ".$_SERVER["PHP_SELF"]);
        exit;
    }
}
...
if (GETPOST('save', 'alpha'))
{
	...
	if (! empty($fckeditor_skin)) {
		if (! mounir_set_const($db, 'FCKEDITOR_SKIN', $fckeditor_skin, 'chaine', 0, '', $conf->entity)) {
			$error ++;
		}
	} else {
		$error ++;
    ...
    if (! empty($fckeditor_test)) {
		if (! mounir_set_const($db, 'FCKEDITOR_TEST', $fckeditor_test, 'chaine', 0, '', $conf->entity)) {
			$error ++;
		}
	} else {
		$error ++;
...
else
{
    ...
    $listofmodes=array('mounir_mailings', 'mounir_notes', 'mounir_details', 'mounir_readonly', 'Full', 'Full_inline');
    $linkstomode='';
    ...
    if ($mode != 'Full_inline')
    {
        ...
        $readonly=($mode=='mounir_readonly'?1:0);
