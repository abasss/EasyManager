...
if (empty($reshook))
{
	if ($action == 'setvalue' && $user->admin)
	{
		...
		$db->begin();
		if (! mounir_set_const($db, 'LDAP_SERVER_TYPE', GETPOST("type"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_SERVER_PROTOCOLVERSION', GETPOST("LDAP_SERVER_PROTOCOLVERSION"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_SERVER_HOST', GETPOST("host"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_SERVER_HOST_SLAVE', GETPOST("slave"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_SERVER_PORT', GETPOST("port"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_SERVER_DN', GETPOST("dn"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_ADMIN_DN', GETPOST("admin"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_ADMIN_PASS', GETPOST("pass"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_SERVER_USE_TLS', GETPOST("usetls"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_SYNCHRO_ACTIVE', GETPOST("activesynchro"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_CONTACT_ACTIVE', GETPOST("activecontact"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_MEMBER_ACTIVE', GETPOST("activemembers"), 'chaine', 0, '', $conf->entity)) $error++;
		if (! mounir_set_const($db, 'LDAP_MEMBER_TYPE_ACTIVE', GETPOST("activememberstypes"), 'chaine', 0, '', $conf->entity)) $error++;
...
$arraylist['ldap2mounir']=$langs->trans("LDAPToMounir");
$arraylist['mounir2ldap']=$langs->trans("MounirToLDAP");
...
if (! empty($conf->societe->enabled))
{
	...
	$arraylist=array();
	$arraylist['0']=$langs->trans("No");
	$arraylist['1']=$langs->trans("MounirToLDAP");
...
if (! empty($conf->adherent->enabled))
{
	...
	$arraylist['0']=$langs->trans("No");
	$arraylist['1']=$langs->trans("MounirToLDAP");
	$arraylist['ldap2mounir']=$langs->trans("LDAPToMounir").' ('.$langs->trans("SupportedForLDAPImportScriptOnly").')';
