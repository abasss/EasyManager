...
{
	if (! mounir_set_const($db, 'USER_PASSWORD_GENERATED', $_GET["value"], 'chaine', 0, '', $conf->entity))
	{
		dol_print_error($db);
	}
	else
...
if ($action == 'activate_encrypt')
{
    ...
    mounir_set_const($db, "DATABASE_PWD_ENCRYPTED", "1", 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'disable_encrypt')
{
	...
	{
		mounir_del_const($db, "DATABASE_PWD_ENCRYPTED", $conf->entity);
    }
	header("Location: security.php");
    exit;
}
...
if ($action == 'activate_encryptdbpassconf')
{
	...
	if ($result > 0)
	{
		...
		//mounir_set_const($db, "MAIN_DATABASE_PWD_CONFIG_ENCRYPTED", "1");
		header("Location: security.php");
		exit;
	}
	else
	{
		setEventMessages($langs->trans('InstrucToEncodePass', dol_encode($mounir_main_db_pass)), null, 'warnings');
	}
}
elseif ($action == 'disable_encryptdbpassconf')
{
	...
	if ($result > 0)
	{
		...
		// database value not required
		//mounir_del_const($db, "MAIN_DATABASE_PWD_CONFIG_ENCRYPTED",$conf->entity);
		header("Location: security.php");
		exit;
	}
	else
	{
		setEventMessages($langs->trans('InstrucToClearPass', $mounir_main_db_pass), null, 'warnings');
	}
}

if ($action == 'activate_MAIN_SECURITY_DISABLEFORGETPASSLINK')
{
	mounir_set_const($db, "MAIN_SECURITY_DISABLEFORGETPASSLINK", '1', 'chaine', 0, '', $conf->entity);
	header("Location: security.php");
	exit;
}
elseif ($action == 'disable_MAIN_SECURITY_DISABLEFORGETPASSLINK')
{
	mounir_del_const($db, "MAIN_SECURITY_DISABLEFORGETPASSLINK", $conf->entity);
	header("Location: security.php");
	exit;
}
...
if ($action == 'maj_pattern')
{
    ...
    if (!$patternInError) {
	    mounir_set_const($db, "USER_PASSWORD_PATTERN", $pattern, 'chaine', 0, '', $conf->entity);
	    header("Location: security.php");
	    exit;
    }
}
...
print '<td align="center" width="100">';
if (empty($mounir_main_db_pass) && empty($mounir_main_db_encrypted_pass))
{
	$langs->load("errors");
...
{
	if (empty($mounir_main_db_encrypted_pass))
	{
