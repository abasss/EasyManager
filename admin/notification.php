 ...
 * Copyright (C) 2015      Bahfir Abbes         <contact@mounirpar.org>
 *
...
if ($action == 'setvalue' && $user->admin)
{
	...
	$result=mounir_set_const($db, "NOTIFICATION_EMAIL_FROM", $_POST["email_from"], 'chaine', 0, '', $conf->entity);
    if ($result < 0) $error++;
    ...
    if (! $error && is_array($_POST))
    {
	    ...
	    foreach($_POST as $key => $val)
	    {
	    	...
	    	{
				mounir_del_const($db, 'NOTIFICATION_FIXEDEMAIL_'.$reg[1].'_THRESHOLD_HIGHER_'.$reg[2], $conf->entity);
	    	...
	    	if ($newkey && $newval)
	    	{
				$result=mounir_set_const($db, $newkey, $newval, 'chaine', 0, '', $conf->entity);
	    	}
	    }
    }
