...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->CONTRACT_ADDON_PDF == "$value") mounir_del_const($db, 'CONTRACT_ADDON_PDF', $conf->entity);
	}
}
...
{
	if (mounir_set_const($db, "CONTRACT_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'setmod')
{
	...
	mounir_set_const($db, "CONTRACT_ADDON", $value, 'chaine', 0, '', $conf->entity);
}

elseif ($action == 'set_other')
{
	$freetext= GETPOST('CONTRACT_FREE_TEXT', 'none');	// No alpha here, we want exact string
	$res1 = mounir_set_const($db, "CONTRACT_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);

	$draft= GETPOST('CONTRACT_DRAFT_WATERMARK', 'alpha');
	$res2 = mounir_set_const($db, "CONTRACT_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
