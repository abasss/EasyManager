...
if ($action == 'set')
{
		...
		if (in_array($syslogHandler, $syslogModules))
		{
			...
			foreach ($module->configure() as $option)
			{
				if (isset($_POST[$option['constant']]))
				{
					...
					mounir_del_const($db, $option['constant'], -1);
					mounir_set_const($db, $option['constant'], $_POST[$option['constant']], 'chaine', 0, '', 0);
				}
			}
		}
	}
    ...
    mounir_del_const($db, 'SYSLOG_HANDLERS', -1);  // To be sure ther is not a setup into another entity
    mounir_set_const($db, 'SYSLOG_HANDLERS', json_encode($activeModules), 'chaine', 0, '', 0);
...
{
	$level = GETPOST("level");
	$res = mounir_set_const($db, "SYSLOG_LEVEL", $level, 'chaine', 0, '', 0);
	...
	if (! $error)
	{
		...
		$res = mounir_set_const($db, "SYSLOG_FILE_SAVES", $file_saves, 'chaine', 0, '', 0);
...
$syslogfacility=$defaultsyslogfacility=mounir_get_const($db, "SYSLOG_FACILITY", 0);
$syslogfile=$defaultsyslogfile=mounir_get_const($db, "SYSLOG_FILE", 0);

if (! $defaultsyslogfacility) $defaultsyslogfacility='LOG_USER';
if (! $defaultsyslogfile) $defaultsyslogfile='brs.log';
