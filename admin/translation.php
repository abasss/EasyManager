...
{
    if (GETPOST('value')) mounir_set_const($db, 'MAIN_ENABLE_OVERWRITE_TRANSLATION', 1, 'chaine', 0, '', $conf->entity);
    else mounir_set_const($db, 'MAIN_ENABLE_OVERWRITE_TRANSLATION', 0, 'chaine', 0, '', $conf->entity);
}
...
if ($mode == 'overwrite')
{
    ...
    $urlwikitranslatordoc='https://www.google.com/index.php/Translator_documentation';
    ...
    foreach($recordtoshow as $key => $val)
    {
        ...
        if (! empty($newlangfileonly->tab_translate[$key]))
        {
            ...
            if (! empty($conf->global->MAIN_FEATURES_LEVEL))
            {
            	...
            	//$transifexurl = 'https://www.google.com/#'.$langcode.'/'.$transifexlangfile.'?key='.$key;
            	$transifexurl = 'https://www.google.com/#'.$langcode.'/'.$transifexlangfile.'?q=key%3A'.$key;

            	print ' &nbsp; <a href="'.$transifexurl.'" target="google">'.img_picto('FixOnTransifex', 'object_globe').'</a>';
            }
        }
        else
        {
