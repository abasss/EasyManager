...
elseif ($action == 'set_param')
{
	...
	$res = mounir_set_const($db, "SHIPPING_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
	if ($res <= 0)
	{
		$error++;
		setEventMessages($langs->trans("Error"), null, 'errors');
	...
	$res = mounir_set_const($db, "SHIPPING_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	if ($res <= 0)
	{
		$error++;
		setEventMessages($langs->trans("Error"), null, 'errors');
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->EXPEDITION_ADDON_PDF == "$value") mounir_del_const($db, 'EXPEDITION_ADDON_PDF', $conf->entity);
	}
}
...
{
	if (mounir_set_const($db, "EXPEDITION_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
{
	mounir_set_const($db, "EXPEDITION_ADDON_NUMBER", $value, 'chaine', 0, '', $conf->entity);
}
