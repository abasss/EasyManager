...
$urlmounirmodules='https://www.dolistore.com/';
...
if ($action=='install')
{
    ...
    else
    {
        ...
        {
            $langs->load("errors");
            setEventMessages($langs->trans("ErrorFileMustBeAMounirPackage", $original_file), null, 'errors');
            $error++;
        }
    	if (! $error && ! preg_match('/^(module[a-zA-Z0-9]*|theme)_.*\-([0-9][0-9\.]*)\.zip$/i', $original_file))
		{
			$langs->load("errors");
			setEventMessages($langs->trans("ErrorFilenameDosNotMatchMounirPackageRules", $original_file, 'module_*-x.y*.zip'), null, 'errors');
			$error++;
		}
...
if ($mode == 'common')
{
    ...
    foreach ($orders as $key => $value)
    {
        ...
        $alttext='';
        //if (is_array($objMod->need_mounir_version)) $alttext.=($alttext?' - ':'').'ERP >= '.join('.',$objMod->need_mounir_version);
...
if ($mode == 'deploy')
{
	...
	$allowfromweb=1;
	if (dol_is_file($mounirdataroot.'/installmodules.lock')) $allowonlineinstall=false;

	$fullurl='<a href="'.$urlmounirmodules.'" target="_blank">'.$urlmounirmodules.'</a>';
	$message='';
	if (! empty($allowonlineinstall))
	{
		if (! in_array('/custom', explode(',', $mounir_main_url_root_alt)))
		{
	...
	{
		$message=info_admin($langs->trans("InstallModuleFromWebHasBeenDisabledByFile", $mounirdataroot.'/installmodules.lock'));
		$allowfromweb=0;
	}
...
if ($mode == 'develop')
{
	...
	//print '<img border="0" class="imgautosize imgmaxwidth180" src="'.DOL_URL_ROOT.'/theme/mounir_preferred_partner_int.png">';
	...
	print '<tr class="oddeven" height="80">'."\n";
	$url='https://partners.mounir.org';
	print '<td class="left">';
	print'<a href="'.$url.'" target="_blank" rel="external"><img border="0" class="imgautosize imgmaxwidth180" src="'.DOL_URL_ROOT.'/theme/mounir_preferred_partner_int.png"></a>';
	print '</td>';
