...
if ($action == 'add' || (GETPOST('add') && $action != 'update'))
{
	...
	if (! $error)
	{
		if (mounir_set_const($db, $constname, $constvalue, 'chaine', 1, $constnote, $entity) >= 0)
		{
...
if (! empty($consts) && $action == 'update')
{
	...
	foreach($consts as $const)
	{
		...
		{
			if (mounir_set_const($db, $const["name"], $const["value"], $const["type"], 1, $const["note"], $const["entity"]) >= 0)
			{
				$nbmodified++;
			}
			else
...
if (! empty($consts) && $action == 'delete')
{
	...
	foreach($consts as $const)
	{
		...
		{
			if (mounir_del_const($db, $const["rowid"], -1) >= 0)
			{
				$nbdeleted++;
			}
			else
...
if ($action == 'delete')
{
	if (mounir_del_const($db, $rowid, $entity) >= 0)
	{
