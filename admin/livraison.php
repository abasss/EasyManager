...
if ($action == 'updateMask')
{
    ...
    if ($maskconstdelivery)  $res = mounir_set_const($db, $maskconstdelivery, $maskdelivery, 'chaine', 0, '', $conf->entity);
...
if ($action == 'set_DELIVERY_FREE_TEXT')
{
    ...
    $res=mounir_set_const($db, "DELIVERY_FREE_TEXT", $free, 'chaine', 0, '', $conf->entity);
...
if ($action == 'del')
{
   $ret = delDocumentModel($value, $type);
    if ($ret > 0)
    {
        if ($conf->global->LIVRAISON_ADDON_PDF == "$value") mounir_del_const($db, 'LIVRAISON_ADDON_PDF', $conf->entity);
    }
}

if ($action == 'setdoc')
{
    if (mounir_set_const($db, "LIVRAISON_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
    {
...
if ($action == 'setmod')
{
    ...
    mounir_set_const($db, "LIVRAISON_ADDON_NUMBER", $value, 'chaine', 0, '', $conf->entity);
}
