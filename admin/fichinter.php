...
if ($action == 'updateMask')
{
	...
	if ($maskconst) $res = mounir_set_const($db, $maskconst, $maskvalue, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->FICHEINTER_ADDON_PDF == "$value") mounir_del_const($db, 'FICHEINTER_ADDON_PDF', $conf->entity);
	}
}
...
{
	if (mounir_set_const($db, "FICHEINTER_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'setmod')
{
	...
	mounir_set_const($db, "FICHEINTER_ADDON", $value, 'chaine', 0, '', $conf->entity);
}

elseif ($action == 'set_FICHINTER_FREE_TEXT')
{
	$freetext= GETPOST('FICHINTER_FREE_TEXT', 'none');	// No alpha here, we want exact string
	$res = mounir_set_const($db, "FICHINTER_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'set_FICHINTER_DRAFT_WATERMARK')
{
	...
	$res = mounir_set_const($db, "FICHINTER_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'set_FICHINTER_PRINT_PRODUCTS')
{
	...
	$res = mounir_set_const($db, "FICHINTER_PRINT_PRODUCTS", ($val == 'on' ? 1 : 0), 'bool', 0, '', $conf->entity);
...
$res = mounir_set_const($db, "FICHINTER_USE_SERVICE_DURATION", ($val == 'on' ? 1 : 0), 'bool', 0, '',
		$conf->entity);
	...
	if (!$error) {
        ...
        $res = mounir_set_const($db, "FICHINTER_WITHOUT_DURATION", ($val == 'on' ? 1 : 0), 'bool', 0, '',
                $conf->entity);
        ...
        $res = mounir_set_const($db, "FICHINTER_DATE_WITHOUT_HOUR", ($val == 'on' ? 1 : 0), 'bool', 0, '',
                $conf->entity);
