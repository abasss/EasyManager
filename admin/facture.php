...
if ($action == 'updateMask')
{
    ...
    if ($maskconstinvoice) $res = mounir_set_const($db, $maskconstinvoice, $maskinvoice, 'chaine', 0, '', $conf->entity);
    if ($maskconstreplacement) $res = mounir_set_const($db, $maskconstreplacement, $maskreplacement, 'chaine', 0, '', $conf->entity);
    if ($maskconstcredit)  $res = mounir_set_const($db, $maskconstcredit, $maskcredit, 'chaine', 0, '', $conf->entity);
	if ($maskconstdeposit)  $res = mounir_set_const($db, $maskconstdeposit, $maskdeposit, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->FACTURE_ADDON_PDF == "$value") mounir_del_const($db, 'FACTURE_ADDON_PDF', $conf->entity);
	}
}
// Set default model
elseif ($action == 'setdoc')
{
	if (mounir_set_const($db, "FACTURE_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'setmod')
{
    ...
    mounir_set_const($db, "FACTURE_ADDON", $value, 'chaine', 0, '', $conf->entity);
}
elseif ($action == 'setribchq')
{
	...
	$res = mounir_set_const($db, "FACTURE_RIB_NUMBER", $rib, 'chaine', 0, '', $conf->entity);
    $res = mounir_set_const($db, "FACTURE_CHQ_NUMBER", $chq, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'set_FACTURE_DRAFT_WATERMARK')
{
    ...
    $res = mounir_set_const($db, "FACTURE_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'set_INVOICE_FREE_TEXT')
{
    ...
    $res = mounir_set_const($db, "INVOICE_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'setforcedate')
{
    ...
    $res = mounir_set_const($db, "FAC_FORCE_DATE_VALIDATION", $forcedate, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'setDefaultPDFModulesByType')
{
    ...
    if(!empty($invoicetypemodels) && is_array($invoicetypemodels))
    {
        ...
        {
            $res = mounir_set_const($db, 'FACTURE_ADDON_PDF_'.intval($type), $value, 'chaine', 0, '', $conf->entity);
            if (! $res > 0) $error++;
        }
