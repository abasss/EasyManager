...
if ($action == 'updateMask')
{
	...
	if ($maskconstsupplier_proposal) $res = mounir_set_const($db, $maskconstsupplier_proposal, $masksupplier_proposal, 'chaine', 0, '', $conf->entity);
...
if ($action == 'set_SUPPLIER_PROPOSAL_DRAFT_WATERMARK')
{
	...
	$res = mounir_set_const($db, "SUPPLIER_PROPOSAL_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	if (! $res > 0) $error++;
...
if ($action == 'set_SUPPLIER_PROPOSAL_FREE_TEXT')
{
	...
	$res = mounir_set_const($db, "SUPPLIER_PROPOSAL_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
...
{
    $res = mounir_set_const($db, "BANK_ASK_PAYMENT_BANK_DURING_SUPPLIER_PROPOSAL", $value, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->SUPPLIER_PROPOSAL_ADDON_PDF == "$value") mounir_del_const($db, 'SUPPLIER_PROPOSAL_ADDON_PDF', $conf->entity);
	}
}

elseif ($action == 'setdoc')
{
    if (mounir_set_const($db, "SUPPLIER_PROPOSAL_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'setmod')
{
	...
	mounir_set_const($db, "SUPPLIER_PROPOSAL_ADDON", $value, 'chaine', 0, '', $conf->entity);
}
