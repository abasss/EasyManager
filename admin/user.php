...
elseif ($action == 'del_default')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->USER_ADDON_PDF_ODT == "$value") mounir_del_const($db, 'USER_ADDON_PDF_ODT', $conf->entity);
	}
	$res = true;
}
...
{
	if (mounir_set_const($db, "USER_ADDON_PDF_ODT", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'sethideinactiveuser')
{
	...
	if (mounir_set_const($db, "USER_HIDE_INACTIVE_IN_COMBOBOX", $status, 'chaine', 0, '', $conf->entity) > 0)
	{
		header("Location: ".$_SERVER["PHP_SELF"]);
		exit;
	}
