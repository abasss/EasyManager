...
if ($action == 'setbankorder') {
    if (mounir_set_const($db, "BANK_SHOW_ORDER_OPTION", GETPOST('value', 'alpha'), 'chaine', 0, '', $conf->entity) > 0)
    {
...
if ($action == 'setreportlastnumreleve') {
    if (mounir_set_const($db, "BANK_REPORT_LAST_NUM_RELEVE", 1, 'chaine', 0, '', $conf->entity) > 0)
    {
        header("Location: " . $_SERVER["PHP_SELF"]);
        exit;
    }
...
elseif ($action == 'unsetreportlastnumreleve') {
    if (mounir_set_const($db, "BANK_REPORT_LAST_NUM_RELEVE", 0, 'chaine', 0, '', $conf->entity) > 0)
    {
        header("Location: " . $_SERVER["PHP_SELF"]);
        exit;
    }
...
elseif ($action == 'del') {
    ...
    if ($ret > 0) {
        if ($conf->global->BANKADDON_PDF == "$value")
            mounir_del_const($db, 'BANKADDON_PDF', $conf->entity);
    }
}
// Set default model
elseif ($action == 'setdoc') {
    if (mounir_set_const($db, "BANKADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
