...
if ($action == 'updateMask')
{
	...
	if ($maskconstbom) $res = mounir_set_const($db, $maskconstbom, $maskbom, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->BOM_ADDON_PDF == "$value") mounir_del_const($db, 'BOM_ADDON_PDF', $conf->entity);
	}
}
...
{
	if (mounir_set_const($db, "BOM_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'setmod')
{
	...
	mounir_set_const($db, "BOM_ADDON", $value, 'chaine', 0, '', $conf->entity);
}

elseif ($action == 'set_BOM_DRAFT_WATERMARK')
{
	$draft = GETPOST("BOM_DRAFT_WATERMARK");
	$res = mounir_set_const($db, "BOM_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'set_BOM_FREE_TEXT')
{
	...
	$res = mounir_set_const($db, "BOM_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
    ...
    $res = mounir_set_const($db, "SHIPPABLE_BOM_ICON_IN_LIST", $setshippableiconinlist, 'yesno', 0, '', $conf->entity);
    if (! $res > 0) $error++;
    if (! $error) {
        setEventMessages($langs->trans("SetupSaved"), null, 'mesgs');
    } else {
