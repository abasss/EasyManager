...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->INVOICE_SUPPLIER_ADDON_PDF == "$value") mounir_del_const($db, 'INVOICE_SUPPLIER_ADDON_PDF', $conf->entity);
	}
}
...
{
    if (mounir_set_const($db, "INVOICE_SUPPLIER_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
    {
...
{
    mounir_del_const($db, "INVOICE_SUPPLIER_ADDON_PDF", $conf->entity);
}
...
if ($action == 'setmod')
{
    ...
    mounir_set_const($db, "INVOICE_SUPPLIER_ADDON_NUMBER", $value, 'chaine', 0, '', $conf->entity);
}
...
if ($action == 'set_SUPPLIER_INVOICE_FREE_TEXT')
{
    ...
    $res = mounir_set_const($db, "SUPPLIER_INVOICE_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
