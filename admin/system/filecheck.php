 ...
 *  \brief      Page to check ERP files integrity
 */
...
print load_fiche_titre($langs->trans("FileCheckMounir"), '', 'title_setup');
...
if (! $error && $xml)
{
    ...
    {
        print 'Error: Failed to found mounir_htdocs_dir into XML file '.$xmlfile;
        $error++;
    }
    ...
    // Scan scripts
    /*
    if (is_object($xml->mounir_script_dir[0]))
    {
        $file_list = array();
        $ret = getFilesUpdated($file_list, $xml->mounir_htdocs_dir[0], '', ???, $checksumconcat);		// Fill array $file_list
    }*/
