...
if (! empty($conf->expedition->enabled) && empty($conf->global->MAIN_SUBMODULE_EXPEDITION))
{
    ...
    mounir_set_const($db, "MAIN_SUBMODULE_EXPEDITION", "1", 'chaine', 0, '', $conf->entity);
}
/*
...
{
	mounir_del_const($db, "MAIN_SUBMODULE_EXPEDITION",$conf->entity);
    header("Location: confexped.php");
    exit;
}
*/
...
{
    mounir_set_const($db, "MAIN_SUBMODULE_EXPEDITION", "1", 'chaine', 0, '', $conf->entity);    // We must also enable this
    mounir_set_const($db, "MAIN_SUBMODULE_LIVRAISON", "1", 'chaine', 0, '', $conf->entity);
	header("Location: confexped.php");
	exit;
}
elseif ($action == 'disable_delivery')
{
	mounir_del_const($db, "MAIN_SUBMODULE_LIVRAISON", $conf->entity);
    header("Location: confexped.php");
    exit;
}
