		...
		if ($result >= 0 && ! count($object->errors))
		{
			...
			if ($object->email && GETPOST("send_mail"))
			{
				...
				if (empty($labeltouse) || (int) $labeltouse === -1) {
					...
					$moreinheader='X-ERP-Info: send_an_email by adherents/card.php'."\r\n";
...
else
{
	...
	if ($action == 'edit')
	{
		...
		********************************************/
		...
		{
			print '<tr><td>'.$langs->trans("LinkedToMounirThirdParty").'</td><td colspan="2" class="valeur">';
			if ($object->socid)
			{
				$company=new Societe($db);
		...
		// Login ERP
		print '<tr><td>'.$langs->trans("LinkedToMounirUser").'</td><td colspan="2" class="valeur">';
		if ($object->user_id)
		{
			$form->form_users($_SERVER['PHP_SELF'].'?rowid='.$object->id, $object->user_id, 'none');
		}
		else print $langs->trans("NoMounirAccess");
		print '</td></tr>';
	...
	if ($id > 0 && $action != 'edit')
	{
		...
		if ($action == 'create_user')
		{
			...
			print $form->formconfirm($_SERVER["PHP_SELF"]."?rowid=".$object->id, $langs->trans("CreateMounirLogin"), $text, "confirm_create_user", $formquestion, 'yes');
		}
		...
		if ($action == 'create_thirdparty')
		{
			...
			print $form->formconfirm($_SERVER["PHP_SELF"]."?rowid=".$object->id, $langs->trans("CreateMounirThirdParty"), $langs->trans("ConfirmCreateThirdParty"), "confirm_create_thirdparty", $formquestion, 'yes');
		}
		...
		if (! empty($conf->societe->enabled))
		{
			...
			print $form->editfieldkey('LinkedToMounirThirdParty', 'thirdparty', '', $object, $editenable);
		...
		print $form->editfieldkey('LinkedToMounirUser', 'login', '', $object, $editenable);
		print '</td><td colspan="2" class="valeur">';
		if ($action == 'editlogin')
		{
		...
		if (empty($reshook)) {
			if ($action != 'editlogin' && $action != 'editthirdparty')
			{
				...
				if (! empty($conf->societe->enabled) && ! $object->socid)
				{
					...
					{
						if ($object->statut != -1) print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?rowid='.$object->id.'&amp;action=create_thirdparty">'.$langs->trans("CreateMounirThirdParty").'</a></div>';
						else print '<div class="inline-block divButAction"><a class="butActionRefused classfortooltip" href="#" title="'.dol_escape_htmltag($langs->trans("ValidateBefore")).'">'.$langs->trans("CreateMounirThirdParty").'</a></div>';
					}
					else
					{
						print '<div class="inline-block divButAction"><font class="butActionRefused classfortooltip" href="#" title="'.dol_escape_htmltag($langs->trans("NotEnoughPermissions")).'">'.$langs->trans("CreateMounirThirdParty")."</font></div>";
					}
				}
				...
				if (! $user->societe_id && ! $object->user_id)
				{
					...
					{
						if ($object->statut != -1) print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?rowid='.$object->id.'&amp;action=create_user">'.$langs->trans("CreateMounirLogin").'</a></div>';
						else print '<div class="inline-block divButAction"><a class="butActionRefused classfortooltip" href="#" title="'.dol_escape_htmltag($langs->trans("ValidateBefore")).'">'.$langs->trans("CreateMounirLogin").'</a></div>';
					}
					else
					{
						print '<div class="inline-block divButAction"><font class="butActionRefused classfortooltip" href="#" title="'.dol_escape_htmltag($langs->trans("NotEnoughPermissions")).'">'.$langs->trans("CreateMounirLogin")."</font></div>";
					}
				}
