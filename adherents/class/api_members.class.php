 ...
 */
class Members extends MounirApi
{
    /**
    ...
    public function get($id)
    {
        ...
        if( ! MounirApi::_checkAccessToResource('adherent', $member->id)) {
    ...
    {
        if(! MounirApiAccess::$user->rights->adherent->creer) {
            throw new RestException(401);
        }
        ...
        if ($member->create(MounirApiAccess::$user) < 0) {
    ...
    public function put($id, $request_data = null)
    {
        ...
        if( ! MounirApi::_checkAccessToResource('member', $member->id)) {
            throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
        }
        ...
        foreach($request_data as $field => $value) {
            ...
            if ($field == 'statut') {
                if ($value == '0') {
                    $result = $member->resiliate(MounirApiAccess::$user);
                    if ($result < 0) {
                        throw new RestException(500, 'Error when resiliating member: '.$member->error);
                    }
                } elseif ($value == '1') {
                    $result = $member->validate(MounirApiAccess::$user);
                    if ($result < 0) {
        ...
        if ($member->update(MounirApiAccess::$user) >= 0)
        {
            return $this->get($id);
        }
        else
    ...
    {
        if(! MounirApiAccess::$user->rights->adherent->supprimer) {
            throw new RestException(401);
        }
        $member = new Adherent($this->db);
        $result = $member->fetch($id);
        ...
        if (! $member->delete($member->id, MounirApiAccess::$user)) {
    ...
    public function getSubscriptions($id)
    {
        ...
        if(! MounirApiAccess::$user->rights->adherent->cotisation->lire) {
            throw new RestException(401);
        }
    ...
    {
        if(! MounirApiAccess::$user->rights->adherent->cotisation->creer) {
            throw new RestException(401);
        }
