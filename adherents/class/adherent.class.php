...
class Adherent extends CommonObject
{
	...
	public function initAsSpecimen()
	{
		...
		$this->firstname = 'AUDOUNE';
		$this->gender='man';
		$this->login='dolibspec';
		$this->pass='dolibspec';
		$this->societe = 'AUDOUNE';
		$this->address = 'Rabat';
		$this->zip = '10000';
		$this->town = 'Rabat';
		$this->country_id = 1;
			...
			if (! empty($conf->global->DATABASE_PWD_ENCRYPTED))
			{
				...
				if (empty($conf->global->MAIN_SECURITY_HASH_ALGO))
				{
					if ($this->pass_indatabase_crypted && ! empty($conf->global->LDAP_MEMBER_FIELD_PASSWORD_CRYPTED))	{
						// Create OpenLDAP MD5 password from ERP MD5 password
				...
				while ($i < $num_rows)
				{
					...
					else
					{
						...
						if (! empty($labeltouse) && is_object($arraydefaultmessage) && $arraydefaultmessage->id > 0)
						{
							...
							$moreinheader='X-ERP-Info: sendReminderForExpiredSubscription'."\r\n";
