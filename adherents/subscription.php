    ...
    if (! $error && $action=='subscription')
    {
        ...
        if (! $error)
        {
            ...
            if ($object->email && $sendalsoemail)   // $object is 'Adherent'
            {
                ...
                $moreinheader='X-ERP-Info: send_an_email by adherents/subscription.php'."\r\n";
...
if ($rowid > 0)
{
	...
	if (! empty($conf->societe->enabled))
	{
		...
		print $langs->trans("LinkedToMounirThirdParty");
		print '</td>';
	...
	print '<tr><td>';
	print '<table class="nobordernopadding" width="100%"><tr><td>';
	print $langs->trans("LinkedToMounirUser");
	print '</td>';
    ...
    if (($action == 'addsubscription' || $action == 'create_thirdparty') && $user->rights->adherent->cotisation->creer)
    {
		...
		if ($action == 'create_thirdparty')
		{
			...
			print $form->formconfirm($_SERVER["PHP_SELF"]."?rowid=".$object->id, $langs->trans("CreateMounirThirdParty"), $langs->trans("ConfirmCreateThirdParty"), "confirm_create_thirdparty", $formquestion, 1);
		}
            ...
            if (! empty($conf->banque->enabled) || ! empty($conf->facture->enabled))
            {
                ...
                if (! empty($conf->societe->enabled) && ! empty($conf->facture->enabled))
                {
                    ...
                    else
					{
                    	...
                    	print $langs->trans("CreateMounirThirdParty");
                    	print '</a>)';
                    }
