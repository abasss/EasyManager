...
abstract class ActionsAdherentCardCommon
{
    ...
    public function assign_values(&$action, $id)
    {
        ...
        if ($action == 'view' || $action == 'edit' || $action == 'delete')
        {
			...
			{
				$mounir_user=new User($this->db);
				$result=$mounir_user->fetch($this->object->user_id);
				$this->tpl['mounir_user'] = $mounir_user->getLoginUrl(1);
			}
			else $this->tpl['mounir_user'] = $langs->trans("NoMounirAccess");
        }
        ...
        if ($action == 'create_user')
        {
        	...
        	$this->tpl['action_create_user'] = $form->formconfirm($_SERVER["PHP_SELF"]."?id=".$this->object->id, $langs->trans("CreateMounirLogin"), $langs->trans("ConfirmCreateAdherent"), "confirm_create_user", $formquestion, 'no');
        }
    }
