 ...
 */
class ExpenseReports extends MounirApi
{
    ...
    {
        if(! MounirApiAccess::$user->rights->expensereport->creer) {
        ...
        if ($this->expensereport->create(MounirApiAccess::$user) < 0) {
    ...
    */
    ...
    public function put($id, $request_data = null)
    {
        ...
        if ($this->expensereport->update(MounirApiAccess::$user) > 0)
        {
            return $this->get($id);
        }
        else
    ...
    {
        if(! MounirApiAccess::$user->rights->expensereport->supprimer) {
            throw new RestException(401);
        }
        $result = $this->expensereport->fetch($id);
        if( ! $result ) {
        ...
        if( ! $this->expensereport->delete(MounirApiAccess::$user)) {
    ...
    public function validate($id, $idwarehouse=0)
    {
        ...
        if( ! $this->expensereport->valid(MounirApiAccess::$user, $idwarehouse)) {
