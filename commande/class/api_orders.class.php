 ...
 */
class Orders extends MounirApi
{
    ...
    public function get($id, $contact_list = 1)
    {
		...
		if( ! MounirApi::_checkAccessToResource('commande', $this->commande->id)) {
    ...
    {
        if(! MounirApiAccess::$user->rights->commande->creer) {
        ...
        if ($this->commande->create(MounirApiAccess::$user) < 0) {
    ...
    public function deleteLine($id, $lineid)
    {
        ...
        $updateRes = $this->commande->deleteline(MounirApiAccess::$user, $lineid);
        if ($updateRes > 0) {
            return $this->get($id);
        } else {
   ...
   /**
    ...
    public function put($id, $request_data = null)
    {
        ...
        if ($this->commande->update(MounirApiAccess::$user) > 0)
        {
            return $this->get($id);
        }
        else
    ...
    {
        if(! MounirApiAccess::$user->rights->commande->supprimer) {
			throw new RestException(401);
		}
        $result = $this->commande->fetch($id);
        if( ! $result ) {
        ...
        if( ! $this->commande->delete(MounirApiAccess::$user)) {
    ...
    public function validate($id, $idwarehouse = 0, $notrigger = 0)
    {
		...
		$result = $this->commande->valid(MounirApiAccess::$user, $idwarehouse, $notrigger);
		if ($result == 0) {
    ...
    public function reopen($id)
    {
        ...
        $result = $this->commande->set_reopen(MounirApiAccess::$user);
        if( $result < 0) {
            throw new RestException(405, $this->commande->error);
        }elseif( $result == 0) {
    ...
    public function setinvoiced($id)
    {
        ...
        $result = $this->commande->classifyBilled(MounirApiAccess::$user);
        if( $result < 0) {
            throw new RestException(400, $this->commande->error);
        }
    ...
    public function close($id, $notrigger = 0)
    {
    	...
    	$result = $this->commande->cloture(MounirApiAccess::$user, $notrigger);
    	if ($result == 0) {
    ...
    public function settodraft($id, $idwarehouse = -1)
    {
        ...
        $result = $this->commande->setDraft(MounirApiAccess::$user, $idwarehouse);
        if ($result == 0) {
    ...
    public function createOrderFromProposal($proposalid)
    {
        ...
        if(! MounirApiAccess::$user->rights->propal->lire) {
            throw new RestException(401);
        }
        if(! MounirApiAccess::$user->rights->commande->creer) {
            throw new RestException(401);
        ...
        $result = $this->commande->createFromProposal($propal, MounirApiAccess::$user);
        if( $result < 0) {
            throw new RestException(405, $this->commande->error);
        }
        $this->commande->fetchObjectLinked();
