 ...
 */
class Invoices extends MounirApi
{
    /**
     *
	...
	{
		if(! MounirApiAccess::$user->rights->facture->lire) {
		...
		if (! MounirApi::_checkAccessToResource('facture', $this->invoice->id)) {
    ...
    public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $thirdparty_ids = '', $status = '', $sqlfilters = '')
    {
        ...
        $socids = MounirApiAccess::$user->societe_id ? MounirApiAccess::$user->societe_id : $thirdparty_ids;

        // If the internal user must only see his customers, force searching by him
        $search_sale = 0;
        if (! MounirApiAccess::$user->rights->societe->client->voir && !$socids) $search_sale = MounirApiAccess::$user->id;

        $sql = "SELECT t.rowid";
        if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socids) || $search_sale > 0) $sql .= ", sc.fk_soc, sc.fk_user"; // We need these fields in order to filter by sale (including the case where the user can only see his prospects)
        $sql.= " FROM ".MAIN_DB_PREFIX."facture as t";

        if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socids) || $search_sale > 0) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc"; // We need this table joined to the select in order to filter by sale

        $sql.= ' WHERE t.entity IN ('.getEntity('invoice').')';
        if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socids) || $search_sale > 0) $sql.= " AND t.fk_soc = sc.fk_soc";
    ...
    {
        if(! MounirApiAccess::$user->rights->facture->creer) {
        ...
        if ($this->invoice->create(MounirApiAccess::$user, 0, (empty($request_data["date_lim_reglement"]) ? 0 : $request_data["date_lim_reglement"])) < 0) {
    ...
    public function createInvoiceFromOrder($orderid)
    {
        ...
        if (! MounirApiAccess::$user->rights->commande->lire) {
            throw new RestException(401);
        }
        if (! MounirApiAccess::$user->rights->facture->creer) {
            throw new RestException(401);
        ...
        $result = $this->invoice->createFromOrder($order, MounirApiAccess::$user);
        if ( $result < 0) {
    ...
    public function postContact($id, $contactid, $type)
    {
        ...
        if(!MounirApi::_checkAccessToResource('invoice', $this->invoice->id)) {
			throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
		}
   ...
   /**
    ...
    public function deleteLine($id, $lineid)
    {
    	...
    	if( ! MounirApi::_checkAccessToResource('facture', $id)) {
    		throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
    	}
    ...
    public function put($id, $request_data = null)
    {
        ...
        if($this->invoice->update(MounirApiAccess::$user))
            return $this->get($id);
    ...
    {
        if(! MounirApiAccess::$user->rights->facture->supprimer) {
			throw new RestException(401);
		}
        $result = $this->invoice->fetch($id);
        if( ! $result ) {
    ...
    public function settodraft($id, $idwarehouse = -1)
    {
        ...
        $result = $this->invoice->setDraft(MounirApiAccess::$user, $idwarehouse);
        if ($result == 0) {
    ...
    public function validate($id, $idwarehouse = 0, $notrigger = 0)
    {
    	...
    	$result = $this->invoice->validate(MounirApiAccess::$user, '', $idwarehouse, $notrigger);
    	if ($result == 0) {
    ...
    public function settopaid($id, $close_code = '', $close_note = '')
    {
        ...
        $result = $this->invoice->set_paid(MounirApiAccess::$user, $close_code, $close_note);
        if ($result == 0) {
            throw new RestException(304, 'Error nothing done. May be object is already validated');
        }
        if ($result < 0) {
    ...
    public function settounpaid($id)
    {
        ...
        $result = $this->invoice->set_unpaid(MounirApiAccess::$user);
        if ($result == 0) {
            throw new RestException(304, 'Nothing done');
        }
        if ($result < 0) {
   ...
   /**
    ...
    public function markAsCreditAvailable($id)
    {
        ...
        if ($canconvert)
        {
            ...
            if ($this->invoice->type == Facture::TYPE_STANDARD || $this->invoice->type == Facture::TYPE_REPLACEMENT || $this->invoice->type == Facture::TYPE_SITUATION)
            {
                ...
                $result = $discount->create(MounirApiAccess::$user);
                if ($result < 0)
                {
                    $error++;
                }
            ...
            if (empty($error))
            {
                if($this->invoice->type != Facture::TYPE_DEPOSIT) {
                    // Classe facture
                    $result = $this->invoice->set_paid(MounirApiAccess::$user);
                    if ($result >= 0)
                    {
                        $this->db->commit();
                    }
    ...
    public function addPayment($id, $datepaye, $paiementid, $closepaidinvoices, $accountid, $num_paiement = '', $comment = '', $chqemetteur = '', $chqbank = '')
    {
    	...
    	if( ! MounirApi::_checkAccessToResource('facture', $id)) {
    		throw new RestException(403, 'Access not allowed for login '.MounirApiAccess::$user->login);
    	}
    	...
    	$paiement_id = $paiement->create(MounirApiAccess::$user, ($closepaidinvoices=='yes'?1:0));    // This include closing invoices
    	if ($paiement_id < 0)
    	{
    		$this->db->rollback();
    	...
    	if (! empty($conf->banque->enabled)) {
    		...
    		$result=$paiement->addPaymentToBank(MounirApiAccess::$user, 'payment', $label, $accountid, $chqemetteur, $chqbank);
    		if ($result < 0)
    		{
    			$this->db->rollback();
    ...
    public function addPaymentDistributed($arrayofamounts, $datepaye, $paiementid, $closepaidinvoices, $accountid, $num_paiement = '', $comment = '', $chqemetteur = '', $chqbank = '')
    {
        ...
        foreach($arrayofamounts as $id => $amount) {
        	...
        	if( ! MounirApi::_checkAccessToResource('facture', $id)) {
        		throw new RestException(403, 'Access not allowed on invoice ID '.$id.' for login '.MounirApiAccess::$user->login);
        	}
        }
