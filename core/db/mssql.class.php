...
/**
 *	Classe de gestion de la database de mounir
 */
class DoliDBMssql extends DoliDB
{
    ...
    public function __construct($type, $host, $user, $pass, $name = '', $port = 0)
	{
		...
		if ($this->db)
		{
			// Si client connecte avec charset different de celui de la base ERP
			// (La base ERP a ete forcee en this->forcecharset a l'install)
			$this->connected = true;
			$this->ok = true;
		}
		else
    ...
    public function connect($host, $login, $passwd, $name, $port = 0)
	{
		...
		// les nouvelles version de ERP car force par l'install ERP.
    ...
    public function errno()
	{
		...
		{
			// Constants to convert a MSSql error code to a generic ERP error code
			$errorcode_map = array(
	...
	{
        // phpcs:enable
        $sql = "CREATE LOGIN ".$this->EscapeFieldName($mounir_main_db_user)." WITH PASSWORD='$mounir_main_db_pass'";
        ...
        $sql="SELECT name from sys.databases where name='".$mounir_main_db_name."'";
        ...
        else
        {
            if ($num)
            {
                $this->select_db($mounir_main_db_name);
                $sql="CREATE USER [$mounir_main_db_user] FOR LOGIN [$mounir_main_db_user]";
                $this->query($sql);
                $sql="ALTER ROLE [db_owner] ADD MEMBER [$mounir_main_db_user]";
                $this->query($sql);
            }
        }
	    return 1;
