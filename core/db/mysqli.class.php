 ...
 *	\brief      Class file to manage ERP database access for a MySQL database
 */

require_once DOL_DOCUMENT_ROOT .'/core/db/DoliDB.class.php';

/**
 *	Class to manage ERP database access for a MySQL database using the MySQLi extension
 */
class DoliDBMysqli extends DoliDB
{
    ...
    public function __construct($type, $host, $user, $pass, $name = '', $port = 0)
    {
        ...
        if (! empty($conf->db->mounir_main_db_collation)) $this->forcecollate=$conf->db->mounir_main_db_collation;
        ...
        if ($this->connected && $name)
        {
            if ($this->select_db($name))
            {
                ...
                $clientmustbe=empty($conf->db->mounir_main_db_character_set)?'utf8':$conf->db->mounir_main_db_character_set;
				...
				if ($this->db->character_set_name() != $clientmustbe) {
					...
					$collation = $conf->db->mounir_main_db_collation;
    ...
    public function errno()
    {
        if (! $this->connected) {
            ...
            // Constants to convert a MySql error code to a generic ERP error code
            $errorcode_map = array(
    ...
    public function encrypt($fieldorvalue, $withQuotes = 0)
    {
        ...
        $cryptType = (!empty($conf->db->mounir_main_db_encryption)?$conf->db->mounir_main_db_encryption:0);

        //Encryption key
        $cryptKey = (!empty($conf->db->mounir_main_db_cryptkey)?$conf->db->mounir_main_db_cryptkey:'');
    ...
    public function DDLCreateDb($database, $charset = '', $collation = '', $owner = '')
    {
        ...
        // ALTER DATABASE mounir_db DEFAULT CHARACTER SET latin DEFAULT COLLATE latin1_swedish_ci
	 ...
	 *
	 *	@param	string	$mounir_main_db_host 		Ip server or '%'
	 ...
	 *	@return	int									<0 if KO, >=0 if OK
     */
    public function DDLCreateUser($mounir_main_db_host, $mounir_main_db_user, $mounir_main_db_pass, $mounir_main_db_name)
    {
        // phpcs:enable
        $sql = "CREATE USER '".$this->escape($mounir_main_db_user)."'";
        ...
        $sql = "CREATE USER '".$this->escape($mounir_main_db_user)."'@'localhost'";
        $resql=$this->query($sql);

        $sql = "GRANT ALL PRIVILEGES ON ".$this->escape($mounir_main_db_name).".* TO '".$this->escape($mounir_main_db_user)."'@'".$this->escape($mounir_main_db_host)."' IDENTIFIED BY '".$this->escape($mounir_main_db_pass)."'";
        dol_syslog(get_class($this)."::DDLCreateUser", LOG_DEBUG);	// No sql to avoid password in log
        $resql=$this->query($sql);
        if (! $resql)
        {
            $this->error = "Connected user not allowed to GRANT ALL PRIVILEGES ON ".$this->escape($mounir_main_db_name).".* TO '".$this->escape($mounir_main_db_user)."'@'".$this->escape($mounir_main_db_host)."'  IDENTIFIED BY '*****'";
            return -1;
        }
