...
/**
 *      \file       htdocs/core/login/functions_mounir.php
 *      \ingroup    core
 *      \brief      Authentication functions for ERP mode (check user on login or email and check pass)
 */
 ...
 */
function check_user_password_mounir($usertotest, $passwordtotest, $entitytotest = 1)
{
	...
	if (! empty($usertotest))
	{
		dol_syslog("functions_mounir::check_user_password_mounir usertotest=".$usertotest." passwordtotest=".preg_replace('/./', '*', $passwordtotest)." entitytotest=".$entitytotest);
		...
		if ($resql)
		{
			...
			if ($obj)
			{
				...
				if ($cryptType == 'md5')
				{
					...
					{
						$passok=true;
						dol_syslog("functions_mounir::check_user_password_mounir Authentification ok - ".$cryptType." of pass is ok");
					}
				}
				...
				if (! $passok)
				{
					...
					{
						$passok=true;
						dol_syslog("functions_mounir::check_user_password_mounir Authentification ok - found pass in database");
					}
				}
				...
				else
				{
				    ...
				    dol_syslog("functions_mounir::check_user_password_mounir Authentification ko bad password for '".$usertotest."', cryptType=".$cryptType);
				...
				if ($passok && ! empty($conf->multicompany->enabled))	// We must check entity
				{
					...
					else
					{
						...
						if ($ret < 0)
						{
							dol_syslog("functions_mounir::check_user_password_mounir Authentification ko entity '" . $entitytotest . "' not allowed for user '" . $obj->rowid . "'");
			...
			{
				dol_syslog("functions_mounir::check_user_password_mounir Authentification ko user not found for '".$usertotest."'");
				sleep(1);
		...
		{
			dol_syslog("functions_mounir::check_user_password_mounir Authentification ko db error for '".$usertotest."' error=".$db->lasterror());
			sleep(1);
