...
function check_user_password_ldap($usertotest, $passwordtotest, $entitytotest)
{
	...
	global $_POST;
	global $mounir_main_auth_ldap_host,$mounir_main_auth_ldap_port;
	global $mounir_main_auth_ldap_version,$mounir_main_auth_ldap_servertype;
	global $mounir_main_auth_ldap_login_attribute,$mounir_main_auth_ldap_dn;
	global $mounir_main_auth_ldap_admin_login,$mounir_main_auth_ldap_admin_pass;
	global $mounir_main_auth_ldap_filter;
	global $mounir_main_auth_ldap_debug;
	...
	if ($usertotest)
	{
		...
		$ldaphost=$mounir_main_auth_ldap_host;
		$ldapport=$mounir_main_auth_ldap_port;
		$ldapversion=$mounir_main_auth_ldap_version;
		$ldapservertype=(empty($mounir_main_auth_ldap_servertype) ? 'openldap' : $mounir_main_auth_ldap_servertype);

		$ldapuserattr=$mounir_main_auth_ldap_login_attribute;
		$ldapdn=$mounir_main_auth_ldap_dn;
		$ldapadminlogin=$mounir_main_auth_ldap_admin_login;
		$ldapadminpass=$mounir_main_auth_ldap_admin_pass;
		$ldapdebug=(empty($mounir_main_auth_ldap_debug) || $mounir_main_auth_ldap_debug=="false" ? false : true);
		...
		$userSearchFilter = "";
		if (empty($mounir_main_auth_ldap_filter)) {
			$userSearchFilter = "(" . $ldapuserattr . "=" . $usertotest . ")";
		} else {
			$userSearchFilter = str_replace('%1%', $usertotest, $mounir_main_auth_ldap_filter);
		}
		...
		if ($result > 0)
		{
			if ($result == 2)	// Connection is ok for user/pass into LDAP
			{
				...
				// ldap2mounir synchronisation
				if ($login && ! empty($conf->ldap->enabled) && $conf->global->LDAP_SYNCHRO_ACTIVE == 'ldap2mounir')	// ldap2mounir synchronisation
				{
						dol_syslog("functions_ldap::check_user_password_ldap Sync ldap2mounir");
						...
						// On recherche le user mounir en fonction de son SID ldap
						...
						if ($resultFetchUser > 0)
						{
							...
							// On verifie si le login a change et on met a jour les attributs mounir
							...
							//$resultUpdate = $usertmp->update_ldap2mounir($ldap);
						}
						unset($usertmp);
				}
