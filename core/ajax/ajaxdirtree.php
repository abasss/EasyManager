...
{
	$fullpathselecteddir=$mounir_main_data_root.'/medias/'.($selecteddir != '/' ? $selecteddir : '');
	$fullpathpreopened=$mounir_main_data_root.'/medias/'.($preopened != '/' ? $preopened : '');
}
				...
				foreach ($files as $file)    // $file can be '.', '..', or 'My dir' or 'My file'
				{
					...
					if ($file != '.' && $file != '..' && ((! empty($val['fullrelativename']) && $val['id'] >= 0) || dol_is_dir($fullpathselecteddir . (preg_match('/\/$/', $fullpathselecteddir)?'':'/') . $file)))
					{
						...
						if (preg_match('/^'.preg_quote($val['fullrelativename'].'/', '/').'/', $preopened))
						{
							...
							elseif ($modulepart == 'medias')
							{
								$newfullpathselecteddir=$mounir_main_data_root.'/medias/'.($val['fullrelativename'] != '/' ? $val['fullrelativename'] : '');
							}
