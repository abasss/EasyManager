...
class CMailFile
{
	...
	{
		global $conf, $mounir_main_data_root;
		...
		$this->related_boundary = 'mul_'.dol_hash(uniqid("mounir2"), 3);	// Force md5 hash (does not contains special chars)

		// On defini alternative_boundary
		$this->alternative_boundary = 'mul_'.dol_hash(uniqid("mounir3"), 3);	// Force md5 hash (does not contains special chars)
		...
		if ($this->msgishtml)
		{
			...
			{
				$findimg = $this->findHtmlImages($mounir_main_data_root.'/medias');
			}
		...
		elseif ($this->sendmode == 'swiftmailer')
		{
            ...
            $headers->addTextHeader('X-ERP-TRACKID', $trackid . '@' . $host);
            $headerID = time() . '.swiftmailer-mounir-' . $trackid . '@' . $host;
	...
	public function dump_mail()
	{
		...
		if (@is_writeable($mounir_main_data_root))	// Avoid fatal error on fopen with open_basedir
		{
			$outputfile=$mounir_main_data_root."/mounir_mail.log";
	...
	public function write_smtpheaders()
	{
		...
		if ($trackid)
		{
			...
			$out.= 'Message-ID: <' . time() . '.phpmail-mounir-'. $trackid . '@' . $host . ">" . $this->eol2;	// Uppercase seems replaced by phpmail
			$out.= 'References: <' . time() . '.phpmail-mounir-'. $trackid . '@' . $host . ">" . $this->eol2;
			$out.= 'X-ERP-TRACKID: ' . $trackid . '@' . $host. $this->eol2;
		}
		else
		{
		...
		$out.= "X-Mailer: ERP version " . DOL_VERSION ." (using php mail)".$this->eol2;
