...
class Utils
{
	...
	{
		global $conf, $langs, $mounir_main_data_root;
		...
		if ($choice=='tempfiles' || $choice=='tempfilesold')
		{
			...
			if ($mounir_main_data_root)
			{
			    $filesarray=dol_dir_list($mounir_main_data_root, "directories", 1, '^temp$', '', 'name', SORT_ASC, 2, 0, '', 1);	// Do not follow symlinks
		...
		if ($choice=='allfiles')
		{
			...
			if ($mounir_main_data_root)
			{
				$filesarray=dol_dir_list($mounir_main_data_root, "all", 0, '', 'install\.lock$', 'name', SORT_ASC, 0, 0, '', 1);
			}
		}
		...
		if ($choice=='logfile')
		{
			...
			if ($mounir_main_data_root)
			{
				$filesarray=dol_dir_list($mounir_main_data_root, "files", 0, '.*\.log[\.0-9]*(\.gz)?$', 'install\.lock$', 'name', SORT_ASC, 0, 0, '', 1);
			}
	...
	{
		global $db, $conf, $langs, $mounir_main_data_root;
		global $mounir_main_db_name, $mounir_main_db_host, $mounir_main_db_user, $mounir_main_db_port, $mounir_main_db_pass;
		...
		if ($file == 'auto')
		{
			...
			$file=$prefix.'_'.$mounir_main_db_name.'_'.dol_sanitizeFileName(DOL_VERSION).'_'.strftime("%Y%m%d%H%M").'.'.$ext;
		}
		...
		if ($type == 'mysql' || $type == 'mysqli')
		{
			...
			//$param=escapeshellarg($mounir_main_db_name)." -h ".escapeshellarg($mounir_main_db_host)." -u ".escapeshellarg($mounir_main_db_user)." -p".escapeshellarg($mounir_main_db_pass);
			$param=$mounir_main_db_name." -h ".$mounir_main_db_host;
			$param.=" -u ".$mounir_main_db_user;
			if (! empty($mounir_main_db_port)) $param.=" -P ".$mounir_main_db_port;
			...
			$paramcrypted=$param;
			$paramclear=$param;
			if (! empty($mounir_main_db_pass))
			{
				$paramcrypted.=' -p"'.preg_replace('/./i', '*', $mounir_main_db_pass).'"';
				$paramclear.=' -p"'.str_replace(array('"','`'), array('\"','\`'), $mounir_main_db_pass).'"';
			}
		...
		if ($type == 'postgresql' || $type == 'pgsql')
		{
			...
			//$param="-F c";
			$param="-F p";
			$param.=" --no-tablespaces --inserts -h ".$mounir_main_db_host;
			$param.=" -U ".$mounir_main_db_user;
			if (! empty($mounir_main_db_port)) $param.=" -p ".$mounir_main_db_port;
			...
			/*if (! empty($mounir_main_db_pass))
			 {
			 $paramcrypted.=" -W".preg_replace('/./i','*',$mounir_main_db_pass);
			 $paramclear.=" -W".$mounir_main_db_pass;
			 }*/
			$paramcrypted.=" -w ".$mounir_main_db_name;
			$paramclear.=" -w ".$mounir_main_db_name;
    ...
    public function compressSyslogs()
    {
		...
		$tabfiles = dol_dir_list(DOL_DATA_ROOT, 'files', 0, '^(mounir_.+|odt2pdf)\.log$'); // Also handle other log files like mounir_install.log
	 ...
	 *	Note about foreign keys constraints: for ERP, since there are a lot of constraints and when imported the tables will be inserted in the dumped order, not in constraints order, then we ABSOLUTELY need to use SET FOREIGN_KEY_CHECKS=0; when importing the sql dump.
	...
	public function backupTables($outputfile, $tables = '*')
	{
		...
		$sqlhead = '';
		$sqlhead .= "-- ".$db::LABEL." dump via php with ERP ".DOL_VERSION."
--
