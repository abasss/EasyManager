...
abstract class CommonObject
{
	...
	{
		global $user, $mounir_main_url_root;
	...
	public function showInputField($val, $key, $value, $moreparam = '', $keysuffix = '', $keyprefix = '', $morecss = 0)
	{
		...
		elseif ($type == 'text')
		{
			if (! preg_match('/search_/', $keyprefix))		// If keyprefix is search_ or search_options_, we must just use a simple text field
			{
				...
				$doleditor=new DolEditor($keyprefix.$key.$keysuffix, $value, '', 200, 'mounir_notes', 'In', false, false, false, ROWS_5, '90%');
		...
		elseif ($type == 'html')
		{
			...
			{
				require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
				$doleditor=new DolEditor($keyprefix.$key.$keysuffix, $value, '', 200, 'mounir_notes', 'In', false, false, ! empty($conf->fckeditor->enabled) && $conf->global->FCKEDITOR_ENABLE_SOCIETE, ROWS_5, '90%');
				$out=$doleditor->Create(1);
			}
			else
			{
		...
		elseif ($type == 'checkbox')
		{
			...
			if (is_array($value_arr) && count($value_arr)>0)
			{
				...
				foreach ($value_arr as $keyval=>$valueval) {
					$toprint[]='<li class="select2-search-choice-mounir noborderoncategories" style="background: #aaa">'.$param['options'][$valueval].'</li>';
				}
				$value='<div class="select2-container-multi-mounir" style="width: 90%;"><ul class="select2-choices-mounir">'.implode(' ', $toprint).'</ul></div>';
			}
		}
		elseif ($type == 'chkbxlst')
		{
					...
					if (is_array($value_arr) && in_array($obj->rowid, $value_arr)) {
						if (is_array($fields_label) && count($fields_label) > 1) {
							foreach ($fields_label as $field_toshow) {
								...
								if ($translabel != $field_toshow) {
									$toprint[]='<li class="select2-search-choice-mounir noborderoncategories" style="background: #aaa">'.dol_trunc($translabel, 18).'</li>';
								} else {
									$toprint[]='<li class="select2-search-choice-mounir noborderoncategories" style="background: #aaa">'.$obj->$field_toshow.'</li>';
								}
							}
						} else {
							$translabel = '';
							...
							if ($translabel != $obj->{$InfoFieldList[1]}) {
								...
								$toprint[]='<li class="select2-search-choice-mounir noborderoncategories" style="background: #aaa">'.$obj->{$InfoFieldList[1]}.'</li>';
							}
						}
					}
				}
