...
class Form
{
	 ...
	 * @param	string	$typeofdata		Type of data ('string' by default, 'email', 'amount:99', 'numeric:99', 'text' or 'textarea:rows:cols', 'datepicker' ('day' do not work, don't know why), 'ckeditor:mounir_zzz:width:height:savemethod:1:rows:cols', 'select;xxx[:class]'...)
	 ...
	 * @param	string	$typeofdata		Type of data ('string' by default, 'email', 'amount:99', 'numeric:99', 'text' or 'textarea:rows:cols%', 'datepicker' ('day' do not work, don't know why), 'dayhour' or 'datepickerhour', 'ckeditor:mounir_zzz:width:height:savemethod:toolbarstartexpanded:rows:cols', 'select;xkey:xval,ykey:yval,...')
    ...
    public function editfieldval($text, $htmlname, $value, $object, $perm, $typeofdata = 'string', $editvalue = '', $extObject = null, $custommsg = null, $moreparam = '', $notabletag = 0, $formatfunc = '', $paramid = 'id')
	{
		...
		else
		{
			if (GETPOST('action', 'aZ09') == 'edit'.$htmlname)
			{
				...
				{
				    $tmp=explode(':', $typeofdata);		// Example: ckeditor:mounir_zzz:width:height:savemethod:toolbarstartexpanded:rows:cols:uselocalbrowser
					require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
					$doleditor=new DolEditor($htmlname, ($editvalue?$editvalue:$value), ($tmp[2]?$tmp[2]:''), ($tmp[3]?$tmp[3]:'100'), ($tmp[1]?$tmp[1]:'mounir_notes'), 'In', ($tmp[5]?$tmp[5]:0), (isset($tmp[8])?($tmp[8]?true:false):true), true, ($tmp[6]?$tmp[6]:'20'), ($tmp[7]?$tmp[7]:'100'));
	 ...
	 * @param	string	$inputType		Type of input ('string', 'numeric', 'datepicker' ('day' do not work, don't know why), 'textarea:rows:cols', 'ckeditor:mounir_zzz:width:height:?:1:rows:cols', 'select:loadmethod:savemethod:buttononly')
	...
	public static function selectArrayAjax($htmlname, $url, $id = '', $moreparam = '', $moreparamtourl = '', $disabled = 0, $minimumInputLength = 1, $morecss = '', $callurlonselect = 0, $placeholder = '', $acceptdelayedhtml = 0)
	{
		...
		// TODO Use an internal mounir component instead of select2
    ...
    public function showCategories($id, $type, $rendermode = 0)
	{
		...
		if ($rendermode == 1)
		{
			...
			foreach($categories as $c)
			{
				...
				foreach($ways as $way)
				{
					$toprint[] = '<li class="select2-search-choice-mounir noborderoncategories"'.($c->color?' style="background: #'.$c->color.';"':' style="background: #aaa"').'>'.img_object('', 'category').' '.$way.'</li>';
				}
			}
			return '<div class="select2-container-multi-mounir" style="width: 90%;"><ul class="select2-choices-mounir">'.implode(' ', $toprint).'</ul></div>';
		}
