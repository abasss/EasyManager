...
{
	global $conf, $db, $user;
	global $mounir_main_data_root, $mounir_main_document_root_alt;
	...
	if ($modulepart == 'medias' && !empty($mounir_main_data_root))
	{
	...
	// Wrapping for *.log files, like when used with url http://.../document.php?modulepart=logs&file=brs.log
	elseif ($modulepart == 'logs' && !empty($mounir_main_data_root))
	{
		$accessallowed=($user->admin && basename($original_file) == $original_file && preg_match('/^mounir.*\.log$/', basename($original_file)));
		$original_file=$mounir_main_data_root.'/'.$original_file;
	}
	// Wrapping for *.zip files, like when used with url http://.../document.php?modulepart=packages&file=module_myfile.zip
	elseif ($modulepart == 'packages' && !empty($mounir_main_data_root))
	{
		// Dir for custom dirs
...
function getFilesUpdated(&$file_list, SimpleXMLElement $dir, $path = '', $pathref = '', &$checksumconcat = array())
{
	...
	foreach ($dir->md5file as $file)    // $file is a simpleXMLElement
	{
		...
		else
		{
			...
			if ($conffile == '/etc/mounir/conf.php' && $filename == '/filefunc.inc.php')	// For install with deb or rpm, we ignore test on filefunc.inc.php that was modified by package
			{
