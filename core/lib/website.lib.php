...
function dolWebsiteReplacementOfLinks($website, $content, $removephppart = 0)
{
	...
	// Replace relative link / with mounir URL
	...
	// Replace relative link /xxx.php with mounir URL
	...
	// <img src="medias/image.png... => <img src="mounir/viewimage.php/modulepart=medias&file=image.png...
	$content = preg_replace('/(<img[^>]*src=")(medias\/)/', '\1'.DOL_URL_ROOT.'/viewimage.php?modulepart=medias&file=', $content, -1, $nbrep);
	// <img src="image.png... => <img src="mounir/viewimage.php/modulepart=medias&file=image.png...
	...
	// <img src="viewimage.php/modulepart=medias&file=image.png" => <img src="mounir/viewimage.php/modulepart=medias&file=image.png"
	$content = preg_replace('/(<img[^>]*src=")(\/?viewimage\.php)/', '\1'.DOL_URL_ROOT.'/viewimage.php', $content, -1, $nbrep);

	// action="newpage.php" => action="mounir/website/index.php?website=...&pageref=newpage
 ...
 * Used to ouput the page when viewed from server (ERP or Apache).
 *
...
function dolWebsiteOutput($content, $contenttype = 'html')
{
	...
	global $mounir_main_url_root, $mounir_main_data_root;

	dol_syslog("dolWebsiteOutput start (contenttype=".$contenttype." USEMOUNIRSERVER=".(defined('USEMOUNIRSERVER')?'1':'')." USEMOUNIREDITOR=".(defined('USEMOUNIREDITOR')?'1':'').')');
	...
	if (defined('USEMOUNIREDITOR'))		// REPLACEMENT OF LINKS When page called from ERP editor
	{
	...
	elseif (defined('USEMOUNIRSERVER'))	// REPLACEMENT OF LINKS When page called from ERP server
	{
		global $website;

		// Replace relative link / with mounir URL:  ...href="/"...
		$content=preg_replace('/(href=")\/\"/', '\1'.DOL_URL_ROOT.'/public/website/index.php?website='.$website->ref.'"', $content, -1, $nbrep);
		// Replace relative link /xxx.php with mounir URL:  ...href="....php"
		$content=preg_replace('/(href=")\/?([^:\"]*)(\.php\")/', '\1'.DOL_URL_ROOT.'/public/website/index.php?website='.$website->ref.'&pageref=\2"', $content, -1, $nbrep);
		// Replace relative link /xxx with mounir URL:  ...href="....php"
		...
		// Fix relative link /document.php with correct URL after the DOL_URL_ROOT:  href="/document.php?modulepart=" => href="/mounir/document.php?modulepart="
		$content=preg_replace('/(href=")(\/?document\.php\?[^\"]*modulepart=[^\"]*)(\")/', '\1'.DOL_URL_ROOT.'\2\3', $content, -1, $nbrep);
		$content=preg_replace('/(src=")(\/?document\.php\?[^\"]*modulepart=[^\"]*)(\")/', '\1'.DOL_URL_ROOT.'\2\3', $content, -1, $nbrep);

		// Fix relative link /viewimage.php with correct URL after the DOL_URL_ROOT: href="/viewimage.php?modulepart=" => href="/mounir/viewimage.php?modulepart="
	...
	else									// REPLACEMENT OF LINKS When page called from virtual host
	{
		...
		// <img alt="" src="/mounir_dev/htdocs/viewimage.php?modulepart=medias&amp;entity=1&amp;file=image/ldestailleur_166x166.jpg" style="height:166px; width:166px" />
		// become
...
function dolWebsiteSaveContent($content)
{
	...
	//dol_syslog("dolWebsiteSaveContent start (mode=".(defined('USEMOUNIRSERVER')?'USEMOUNIRSERVER':'').')');
...
function redirectToContainer($containerref, $containeraliasalt = '', $containerid = 0)
{
	...
	if (defined('USEMOUNIRSERVER'))	// When page called from ERP server
	{
 ...
 * @param 	string		$urltograb		URL to grab (exemple: http://www.google.com/ or http://www.google.com/dir1/ or http://www.google.com/dir1/mapage1)
...
function getAllImages($object, $objectpage, $urltograb, &$tmp, &$action, $modifylinks = 0, $grabimages = 1, $grabimagesinto = 'subpage')
{
	...
	$urltograb = dirname($urltograb);							// So urltograb is now http://www.google.com or http://www.google.com/dir1
...
function dolSaveMasterFile($filemaster)
{
	...
	$mastercontent.= "if (! defined('USEMOUNIRSERVER') && ! defined('USEMOUNIREDITOR')) require_once '".DOL_DOCUMENT_ROOT."/master.inc.php';\n";
	$mastercontent.= '?>'."\n";
...
function dolSavePageAlias($filealias, $object, $objectpage)
{
	...
	$aliascontent.= 'global $mounir_main_data_root;'."\n";
	$aliascontent.= 'if (empty($mounir_main_data_root)) require \'./page'.$objectpage->id.'.tpl.php\'; ';
	$aliascontent.= 'else require $mounir_main_data_root.\'/website/\'.$website->ref.\'/page'.$objectpage->id.'.tpl.php\';'."\n";
	$aliascontent.= '?>'."\n";
...
function dolSavePageContent($filetpl, $object, $objectpage)
{
	...
	$tplcontent.= "if (! defined('USEMOUNIRSERVER') && ! defined('USEMOUNIREDITOR')) { require_once './master.inc.php'; } // Not already loaded"."\n";
	...
	$tplcontent.= '<meta name="generator" content="'.DOL_APPLICATION_TITLE.' '.DOL_VERSION.' (https://www.audoune.com)" />'."\n";
	$tplcontent.= '<meta name="mounir:pageid" content="'.dol_string_nohtmltag($objectpage->id).'" />'."\n";
	...
	$tplcontent.= '<!-- File generated by ERP website module editor -->'."\n";
...
function dolSaveIndexPage($pathofwebsite, $fileindex, $filetpl, $filewrapper)
{
	...
	$indexcontent.= "if (! defined('USEMOUNIRSERVER') && ! defined('USEMOUNIREDITOR')) { require_once './master.inc.php'; } // Load master if not already loaded\n";
