 ...
 *  \ingroup    core
 *  \brief		Set of function used for mounir security (not common functions).
 ...
 *	@param		array	$authmode			Array list of selected authentication mode array('http', 'mounir', 'xxx'...)
...
*/
					$function='check_user_password_mounir';
...
if (! function_exists('dol_loginfunction'))
{
    ...
    function dol_loginfunction($langs, $conf, $mysoc)
	{
		...
		$titletruemounirversion=constant('DOL_VERSION');	// $title used by login template after the @ to inform of true ERP version
		...
		*/
		...
		$demologin='';
		$demopassword='';
		if (! empty($mounir_main_demo))
		{
			$tab=explode(',', $mounir_main_demo);
			$demologin=$tab[0];
			$demopassword=$tab[1];
		}
		...
		elseif (is_readable(DOL_DOCUMENT_ROOT.'/theme/'.$conf->theme.'/img/mounir_logo.png'))
		{
			$urllogo=DOL_URL_ROOT.'/theme/'.$conf->theme.'/img/mounir_logo.png';
		}
		elseif (is_readable(DOL_DOCUMENT_ROOT.'/theme/mounir_logo.png'))
		{
...
function encodedecode_dbpassconf($level = 0)
{
	...
	if ($fp = fopen(DOL_DOCUMENT_ROOT.'/conf/conf.php', 'r'))
	{
		while(!feof($fp))
		{
			...
			if (preg_match('/^[^#]*mounir_main_db_encrypted_pass[\s]*=[\s]*(.*)/i', $buffer, $reg))	// Old way to save crypted value
			{
			...
			elseif (preg_match('/^[^#]*mounir_main_db_pass[\s]*=[\s]*(.*)/i', $buffer, $reg))
			{
				$val = trim($reg[1]);	// This also remove CR/LF
				$val=preg_replace('/^["\']/', '', $val);
				$val=preg_replace('/["\'][\s;]*$/', '', $val);
			...
			if ($lineofpass)
			{
				...
				if ($level == 0)
				{
					$config .= '$mounir_main_db_pass=\''.$passwd.'\';'."\n";
				}
				if ($level == 1)
				{
					$config .= '$mounir_main_db_pass=\'crypted:'.$passwd_crypted.'\';'."\n";
				}
