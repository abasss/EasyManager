 ...
 *  To check if ERP version is lower than (x,y,z), do "if versioncompare(versionmounirarray(), array(x.y.z)) <= 0"
 *  For example: if (versioncompare(versionmounirarray(),array(4,0,-4)) >= 0) is true if version is 4.0 alpha or higher.
 *  For example: if (versioncompare(versionmounirarray(),array(4,0,0)) >= 0) is true if version is 4.0 final or higher.
 *  For example: if (versioncompare(versionmounirarray(),array(4,0,1)) >= 0) is true if version is 4.0.1 or higher.
 ...
 *  - Migrate process (mounir-xyz-abc.sql)
            ...
            if (preg_match('/^--\sV(MYSQL|PGSQL)([^\s]*)/i', $buf, $reg))
            {
            	...
            	if ($qualified)
            	{
            		if (! empty($reg[2]))
            		{
            			...
            			{
            				$dbcollation = strtoupper(preg_replace('/_/', '', $conf->db->mounir_main_db_collation));
            				//var_dump($reg[2]);
            				//var_dump($dbcollation);
            				if (empty($conf->db->mounir_main_db_collation) || ($reg[2] != $dbcollation)) $qualified=0;
            				//var_dump($qualified);
            			}
            		}
            	}
 ...
 *	@return     int         			<0 if KO, >0 if OK
 *
 *	@see		mounir_get_const(), mounir_set_const(), dol_set_user_param()
 */
function mounir_del_const($db, $name, $entity = 1)
{
    global $conf;

    if (empty($name))
    {
    	dol_print_error('', 'Error ');// abass
    	return -1;
    }
    ...
    dol_syslog("admin.lib::mounir_del_const", LOG_DEBUG);
    $resql=$db->query($sql);
    if ($resql)
    {
        $conf->global->$name='';
 ...
 *
 *	@see		mounir_del_const(), mounir_set_const(), dol_set_user_param()
 */
function mounir_get_const($db, $name, $entity = 1)
{
    global $conf;
    $value='';
    ...
    dol_syslog("admin.lib::mounir_get_const", LOG_DEBUG);
    $resql=$db->query($sql);
    if ($resql)
    {
        $obj=$db->fetch_object($resql);
 ...
 *
 *	@see		mounir_del_const(), mounir_get_const(), dol_set_user_param()
 */
function mounir_set_const($db, $name, $value, $type = 'chaine', $visible = 0, $note = '', $entity = 1)
{
    global $conf;
    ...
    if (empty($name))
    {
        dol_print_error($db, "Error: Call to function mounir_set_const with wrong parameters", LOG_ERR);
        exit;
    }

    //dol_syslog("mounir_set_const name=$name, value=$value type=$type, visible=$visible, note=$note entity=$entity");
    ...
    dol_syslog("admin.lib::mounir_set_const", LOG_DEBUG);
    $resql=$db->query($sql);
        ...
        while(($file = @readdir($dh)) !== false)
        {
            if (preg_match('/^sess_/i', $file) && $file != "." && $file != "..")
            {
                ...
                if(! @is_dir($fullpath) && is_readable($fullpath))
                {
					...
					// dol_login|s:5:"admin";dol_authmode|s:8:"mounir";dol_tz|s:1:"1";dol_tz_string|s:13:"Europe/Berlin";dol_dst|i:0;dol_dst_observed|s:1:"1";dol_dst_first|s:0:"";dol_dst_second|s:0:"";dol_screenwidth|s:4:"1920";
                    ...
                    if (preg_match('/dol_login/i', $sessValues) && // limit to mounir session
...
function activateModule($value, $withdeps = 1)
{
    ...
    // Test if ERP version ok
    $verdol=versionmounirarray();
    $vermin=isset($objMod->need_mounir_version)?$objMod->need_mounir_version:0;
...
// Abass	
	// if (is_array($vermin) && versioncompare($verdol, $vermin) < 0) {
		// $ret['errors'][] = $langs->trans("ErrorModuleRequireMounirVersion", versiontostring($vermin));
		// return $ret;
	// }
...
function unActivateModule($value, $requiredby = 1)
{
    ...
    else    // We come here when we try to unactivate a module when module does not exists anymore in sources
    {
    	...
    	// TODO Replace this after MounirModules is moved as abstract class with a try catch to show module we try to disable has not been found or could not be loaded
        include_once DOL_DOCUMENT_ROOT.'/core/modules/MounirModules.class.php';
    	$genericMod = new MounirModules($db);
        ...
        if ($result)
        {
            ...
            else
            {
                ...
                elseif ($obj->type == 'html')
                {
                	...
                	$doleditor=new DolEditor('constvalue_'.$const.(empty($strictw3c)?'':'[]'), $obj->value, '', 160, 'mounir_notes', '', false, false, $conf->fckeditor->enabled, ROWS_5, '90%');
                	$doleditor->Create();
                }
