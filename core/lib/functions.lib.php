 ...
 *	\brief			A set of functions for ERP
...
{
    /**
     *  Return a prefix to use for this ERP instance, for session/cookie names or email id.
 ...
 *  @param	int		$type						0=Used for a Filesystem path, 1=Used for an URL path (output relative), 2=Used for an URL path (output full path using same host that current url), 3=Used for an URL path (output full path using host defined into $mounir_main_url_root of conf file)
 ...
 *												On Windows LOG_ERR=4, LOG_WARNING=5, LOG_NOTICE=LOG_INFO=6, LOG_DEBUG=6 si define_syslog_variables ou PHP 5.3+, 7 si mounir
 ...
 *
 *	@param	    string		$domain	    			Domain name (Ex: "yahoo.com", "yhaoo.com", "mounir.fr")
...
function dol_print_error($db = '', $error = '', $errors = null)
{
	...
	{
		$out.=$langs->trans("MounirHasDetectedError").".<br>\n";
		...
		$out.="<b>".$langs->trans("ERP").":</b> ".DOL_VERSION."<br>\n";
	...
	if (empty($mounir_main_prod) && $_SERVER['DOCUMENT_ROOT'] && function_exists('xdebug_print_function_stack') && function_exists('xdebug_call_file'))
	{
	...
	if (empty($mounir_main_prod)) print $out;
	else
	{
		...
		$langs->load("errors");
		print $langs->trans("MounirHasDetectedError").'. ';
		print $langs->trans("YouCanSetOptionMounirMainProdToZero");
		define("MAIN_CORE_ERROR", 1);
	}
	//else print 'Sorry, an error occured but the parameter $mounir_main_prod is defined in conf file so no message is reported to your browser. Please read the log file for error message.';
...
function price2num($amount, $rounding = '', $alreadysqlnb = 0)
{
	...
	if ($alreadysqlnb != 1)	// If not a PHP number or unknown, we change format
	{
		...
		// Convert amount to format with mounir dec and thousand (this is because PHP convert a number
		// to format defined by LC_NUMERIC after a calculation and we want source format to be like defined by ERP setup.
	...
	if ($rounding)
	{
		...
		// Convert amount to format with mounir dec and thousand (this is because PHP convert a number
		// to format defined by LC_NUMERIC after a calculation and we want source format to be defined by ERP setup.
		if (is_numeric($amount))
		{
			// We put in temps value of decimal ("0.00001"). Works with 0 and 2.0E-5 and 9999.10
			$temps=sprintf("%0.10F", $amount-intval($amount));	// temps=0.0000000000 or 0.0000200000 or 9999.1000000000
			$temps=preg_replace('/([\.1-9])0+$/', '\\1', $temps); // temps=0. or 0.00002 or 9999.1
			$nbofdec=max(0, dol_strlen($temps)-2);	// -2 to remove "0."
			$amount=number_format($amount, min($nbofdec, $nbofdectoround), $dec, $thousand);		// Convert amount to format with mounir dec and thousand
		}
		//print "TT".$amount.'<br>';
 ...
 * @deprecated ERP does not support PHP4, you should use native function
 * @see microtime()
 */
					...
					foreach($user->default_values[$relativepathstring]['mandatory'] as $defkey => $defval)
					{
						...
						if ($qualified)
						{
							foreach($defval as $paramkey => $paramval)
							{
								...
								print 'jQuery("select[name=\''.$paramkey.'\']").prop(\'required\',true);'."\n";		// required on a select works only if key is "", this does not happen in ERP
							}
						}
					}
				}
