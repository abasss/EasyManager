 ...
 */
class Products extends MounirApi
{
    /**
    ...
    public function get($id, $ref = '', $ref_ext = '', $barcode = '', $includestockdata = 0)
    {
        ...
        if(! MounirApi::_checkAccessToResource('product', $this->product->id)) {
    ...
    {
        if(! MounirApiAccess::$user->rights->produit->creer) {
        ...
        if ($this->product->create(MounirApiAccess::$user) < 0) {
    ...
    public function put($id, $request_data = null)
    {
        ...
        $result = $this->product->update($id, MounirApiAccess::$user, 1, 'update');
        ...
        if ($result > 0 && ! empty($conf->global->PRODUCT_PRICE_UNIQ)) {
            ...
            if ($pricemodified) {
                ...
                $result = $this->product->updatePrice($newprice, $this->product->price_base_type, MounirApiAccess::$user, $newvat, $newpricemin, 0, $newnpr, 0, 0, array(), $newvatsrccode);
            }
        }
    ...
    {
        if(! MounirApiAccess::$user->rights->produit->supprimer) {
            throw new RestException(401);
        }
        $result = $this->product->fetch($id);
        if(! $result ) {
        ...
        return $this->product->delete(MounirApiAccess::$user);
    }
