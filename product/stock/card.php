...
if ($action == 'create')
{
	...
	$doleditor=new DolEditor('desc', (!empty($object->description)?$object->description:''), '', 180, 'mounir_notes', 'In', false, true, $conf->fckeditor->enabled, ROWS_5, '90%');
	$doleditor->Create();
	print '</td></tr>';
...
else
{
	...
	if ($id > 0 || $ref)
	{
		...
		if (($action == 'edit' || $action == 're-edit') && 1)
		{
			...
			// Editeur wysiwyg
			require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
			$doleditor=new DolEditor('desc', $object->description, '', 180, 'mounir_notes', 'In', false, true, $conf->fckeditor->enabled, ROWS_5, '90%');
			$doleditor->Create();
			print '</td></tr>';
