			...
			if (($action == 'add_price' || $action == 'updateprice' ) && $usercancreate)
			{
				...
				else
				{
					...
					if (empty($reshook))
					{
						...
						{
							print ' - <a href="'.DOL_URL_ROOT.'/societe/card.php?action=create&type=f&backtopage='.urlencode($_SERVER["PHP_SELF"].'?id='.$object->id.'&action='.$action).'">'.$langs->trans("CreateMounirThirdPartySupplier").'</a>';
						}
					}
				}
				print '</td></tr>';
...
SCRIPT;
				...
				if (! empty($conf->global->PRODUIT_FOURN_TEXTS))
				{
    				...
    				$doleditor = new DolEditor('supplier_description', $object->desc_supplier, '', 160, 'mounir_details', '', false, true, $conf->global->FCKEDITOR_ENABLE_PRODUCTDESC, ROWS_4, '90%');
    				$doleditor->Create();
