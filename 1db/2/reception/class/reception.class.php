			...
			if ($resql)
			{
				...
				for ($i = 0; $i < $cpt; $i++)
				{
					...
					if (empty($obj->batch))
					{
						...
						$result=$mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $qty, $obj->subprice, $langs->trans("ReceptionValidatedInMounir", $numref));
						if ($result < 0) {
							$error++;
					...
					else
					{
						...
						$result=$mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $qty, $obj->subprice, $langs->trans("ReceptionValidatedInMounir", $numref), $this->db->jdate($obj->eatby), $this->db->jdate($obj->sellby), $obj->batch);
						if ($result < 0) {
							$error++;
							$this->errors[]=$mouvS->error;
							$this->errors = array_merge($this->errors, $mouvS->errors);
		...
		if ($conf->stock->enabled && $conf->global->STOCK_CALCULATE_ON_RECEPTION && $this->statut > 0)
		{
			...
			if ($resql)
			{
				...
				for ($i = 0; $i < $cpt; $i++)
				{
					...
					$result=$mouvS->livraison($user, $obj->fk_product, $obj->fk_entrepot, $obj->qty, 0, $langs->trans("ReceptionDeletedInMounir", $this->ref), '', $obj->eatby, $obj->sellby, $obj->batch);  // Price is set to 0, because we don't want to see WAP changed
				}
			}
			else
			{
				...
				if ($resql)
				{
					...
					for ($i = 0; $i < $cpt; $i++)
					{
						...
						if (empty($obj->batch))
						{
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result=$mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $qty, $obj->subprice, $langs->trans("ReceptionClassifyClosedInMounir", $numref));
							if ($result < 0) {
						...
						else
						{
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result=$mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $qty, $obj->subprice, $langs->trans("ReceptionClassifyClosedInMounir", $numref), $this->db->jdate($obj->eatby), $this->db->jdate($obj->sellby), $obj->batch);
				...
				if ($resql)
				{
					...
					for ($i = 0; $i < $cpt; $i++)
					{
						...
						if (empty($obj->batch))
						{
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result=$mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, -$qty, $obj->subprice, $langs->trans("ReceptionUnClassifyCloseddInMounir", $numref));
							if ($result < 0) {
							    $this->error = $mouvS->error;
							    $this->errors = $mouvS->errors;
								$error++; break;
						...
						else
						{
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result=$mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, -$qty, $obj->subprice, $langs->trans("ReceptionUnClassifyCloseddInMounir", $numref), $this->db->jdate($obj->eatby), $this->db->jdate($obj->sellby), $obj->batch, $obj->fk_origin_stock);
							if ($result < 0) {
							    $this->error = $mouvS->error;
							    $this->errors = $mouvS->errors;
							    $error++; break;
				...
				if ($resql)
				{
					...
					for ($i = 0; $i < $cpt; $i++)
					{
						...
						if (empty($obj->batch))
						{
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result=$mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, -$qty, $obj->subprice, $langs->trans("ReceptionBackToDraftInMounir", $this->ref));
							if ($result < 0) {
							    $this->error = $mouvS->error;
							    $this->errors = $mouvS->errors;
								$error++;
						...
						else
						{
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result=$mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, -$qty, $obj->subprice, $langs->trans("ReceptionBackToDraftInMounir", $this->ref), $this->db->jdate($obj->eatby), $this->db->jdate($obj->sellby), $obj->batch);
							if ($result < 0) {
							    $this->error = $mouvS->error;
							    $this->errors = $mouvS->errors;
							    $error++; break;
