 ...
 *       \brief      File that is entry point to call ERP WebServices
 */
...
dol_syslog("Call ERP webservices interfaces");
...
{
	$langs->load("admin");
	dol_syslog("Call ERP webservices interfaces with module webservices disabled");
...
$ns='http://www.google.com/ns/';
$server->configureWSDL('WebServicesMounirOther', $ns);
...
$server->wsdl->addComplexType(
    ...
    '',
    array(
        'mounirkey' => array('name'=>'mounirkey','type'=>'xsd:string'),
...
$server->register(
    ...
    // Exit values
    array('result'=>'tns:result','mounir'=>'xsd:string','os'=>'xsd:string','php'=>'xsd:string','webserver'=>'xsd:string'),
    $ns,
    $ns.'#getVersions',
    $styledoc,
    $styleuse,
...
function getVersions($authentication)
{
    ...
    if (! $error)
	{
		...
		$objectresp['mounir']=version_mounir();
