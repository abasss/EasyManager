...
class Setup extends MounirApi
{
     ...
     * code supported by ERP, for example 'en_US' or 'fr_FR'.
     ...
     *
     * @param string	$target			Can be 'local' or 'default' or Url of the signatures file to use for the test. Must be reachable by the tested ERP.
    ...
    public function getCheckIntegrity($target)
    {
    	...
    	if (! MounirApiAccess::$user->admin
    		&& (empty($conf->global->API_LOGIN_ALLOWED_FOR_INTEGRITY_CHECK) || MounirApiAccess::$user->login != $conf->global->API_LOGIN_ALLOWED_FOR_INTEGRITY_CHECK))
    	{
    	...
    	if (empty($xmlremote)) $xmlremote = 'https://www.google.com/files/stable/signatures/filelist-'.DOL_VERSION.'.xml';
    	...
    	if ($xml)
    	{
    		...
    		// Forced constants
    		if (is_object($xml->mounir_constants[0]))
    		{
    			...
    			$i = 0;
    			foreach ($xml->mounir_constants[0]->constant as $constant)    // $constant is a simpleXMLElement
    			{
    		...
    		// Scan htdocs
    		if (is_object($xml->mounir_htdocs_dir[0]))
    		{
    			//var_dump($xml->mounir_htdocs_dir[0]['includecustom']);exit;
    			$includecustom=(empty($xml->mounir_htdocs_dir[0]['includecustom'])?0:$xml->mounir_htdocs_dir[0]['includecustom']);
    			...
    			$ret = getFilesUpdated($file_list, $xml->mounir_htdocs_dir[0], '', DOL_DOCUMENT_ROOT, $checksumconcat, $scanfiles);		// Fill array $file_list
    		...
    		{
    			throw new RestException(500, 'Error: Failed to found mounir_htdocs_dir into XML file '.$xmlfile);
    		}
    		...
    		$checksumtoget = trim((string) $xml->mounir_htdocs_dir_checksum);
