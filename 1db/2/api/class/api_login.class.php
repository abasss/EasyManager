...
class Login
{
    ...
    public function index($login, $password, $entity = '', $reset = 0)
    {
	    ...
	    global $conf, $mounir_main_authentication, $mounir_auto_user;
		...
		if ($mounir_main_authentication == 'forceuser')
		{
			...
			if ($mounir_auto_user != $login)
			{
				dol_syslog("Warning: your instance is set to use the automatic forced login '".$mounir_auto_user."' that is not the requested login. API usage is forbidden in this mode.");
				throw new RestException(403, "Your instance is set to use the automatic login '".$mounir_auto_user."' that is not the requested login. API usage is forbidden in this mode.");
			}
		}
		// Set authmode
		...
		return array(
			'success' => array(
			    ...
			    'message' => 'Welcome ' . $login.($reset?' - Token is new':' - This is your token (generated by a previous call). You can use it to make any REST API call, or enter it into the DOLAPIKEY field to use the ERP API explorer.')
			)
		);
    }
}
