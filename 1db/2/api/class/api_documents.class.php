 ...
 */
class Documents extends MounirApi
{
	...
	public function index($module_part, $original_file = '')
	{
		...
		$check_access = dol_check_secure_access_document($module_part, $original_file, $entity, MounirApiAccess::$user, '', 'read');
	...
	public function builddoc($module_part, $original_file = '', $doctemplate = '', $langcode = '')
	{
		...
		$check_access = dol_check_secure_access_document($module_part, $original_file, $entity, MounirApiAccess::$user, '', 'write');
		$accessallowed              = $check_access['accessallowed'];
		$sqlprotectagainstexternals = $check_access['sqlprotectagainstexternals'];
		$original_file              = $check_access['original_file'];
	...
	public function getDocumentsListByElement($modulepart, $id = 0, $ref = '', $sortfield = '', $sortorder = '')
	{
		...
		elseif ($modulepart == 'adherent' || $modulepart == 'member')
		{
			...
			if (!MounirApiAccess::$user->rights->adherent->lire) {
				throw new RestException(401);
			}
        ...
        elseif ($modulepart == 'produit' || $modulepart == 'product')
		{
			...
			if (!MounirApiAccess::$user->rights->produit->lire) {
				throw new RestException(401);
			}
		...
		elseif ($modulepart == 'agenda' || $modulepart == 'action' || $modulepart == 'event')
		{
			...
			if (!MounirApiAccess::$user->rights->agenda->myactions->read && !MounirApiAccess::$user->rights->agenda->allactions->read) {
				throw new RestException(401);
			}
	...
	public function post($filename, $modulepart, $ref = '', $subdir = '', $filecontent = '', $fileencoding = '', $overwriteifexists = 0)
	{
		...
		if (!MounirApiAccess::$user->rights->ecm->upload) {
			throw new RestException(401);
		}
		...
		// Define $uploadir
		$object = null;
		$entity = MounirApiAccess::$user->entity;
		if ($ref)
		{
			$tmpreldir='';
			...
			$tmp = dol_check_secure_access_document($modulepart, $relativefile, $entity, MounirApiAccess::$user, $ref, 'write');
		...
		else
		{
			...
			$tmp = dol_check_secure_access_document($modulepart, $relativefile, $entity, MounirApiAccess::$user, '', 'write');
			$upload_dir = $tmp['original_file'];	// No dirname here, tmp['original_file'] is already the dir because dol_check_secure_access_document was called with param original_file that is only the dir
