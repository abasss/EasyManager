...
if ($action == 'create')
{
	...
	if ($socid > 0)
	{
        ...
        print '<td colspan="2">';
        $doleditor = new DolEditor('note_public', $note_public, '', 80, 'mounir_notes', 'In', 0, false, true, ROWS_3, '90%');
        ...
        if (empty($user->societe_id))
        {
        	...
        	print '<td colspan="2">';
        	$doleditor = new DolEditor('note_private', $note_private, '', 80, 'mounir_notes', 'In', 0, false, true, ROWS_3, '90%');
        	print $doleditor->Create(1);
		...
		if ($resql)
		{
			...
			while ($i < $num)
			{
				...
				if ($object->statut == 0 && $action == 'editline' && $user->rights->ficheinter->creer && GETPOST('line_id', 'int') == $objp->rowid)
				{
					...
					// Editeur wysiwyg
					require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
					$doleditor=new DolEditor('np_desc', $objp->description, '', 164, 'mounir_details', '', false, true, $conf->global->FCKEDITOR_ENABLE_DETAILS, ROWS_2, '90%');
					$doleditor->Create();
					print '</td>';
			...
			if ($object->statut == 0 && $user->rights->ficheinter->creer && $action <> 'editline' && empty($conf->global->FICHINTER_DISABLE_DETAILS))
			{
                ...
                if (empty($conf->global->FICHINTER_EMPTY_LINE_DESC)) {
                    require_once DOL_DOCUMENT_ROOT . '/core/class/doleditor.class.php';
                    $doleditor = new DolEditor('np_desc', GETPOST('np_desc', 'alpha'), '', 100, 'mounir_details', '', false, true, $conf->global->FCKEDITOR_ENABLE_DETAILS, ROWS_2, '90%');
                    $doleditor->Create();
                }
                print '</td>';
