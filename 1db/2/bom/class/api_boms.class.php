...
class Boms extends MounirApi
{
    ...
    {
		if (! MounirApiAccess::$user->rights->bom->read) {
        ...
        if (! MounirApi::_checkAccessToResource('bom', $this->bom->id, 'bom_bom')) {
    ...
    {
        if(! MounirApiAccess::$user->rights->bom->write) {
            throw new RestException(401);
        }
        ...
        if( ! $this->bom->create(MounirApiAccess::$user)) {
    ...
    public function put($id, $request_data = null)
    {
        ...
        if($this->bom->update($id, MounirApiAccess::$user) > 0)
        {
            return $this->get($id);
        }
        else
    ...
    {
    	if (! MounirApiAccess::$user->rights->bom->delete) {
			throw new RestException(401);
		}
        $result = $this->bom->fetch($id);
        if (! $result) {
		...
		if (! $this->bom->delete(MounirApiAccess::$user))
        {
