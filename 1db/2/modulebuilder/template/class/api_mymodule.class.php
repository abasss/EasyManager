 ...
 */
class MyModuleApi extends MounirApi
{
    /**
    ...
    {
        if (! MounirApiAccess::$user->rights->mymodule->read) {
        ...
        if (! MounirApi::_checkAccessToResource('myobject', $this->myobject->id, 'mymodule_myobject')) {
            throw new RestException(401, 'Access to instance id='.$this->myobject->id.' of object not allowed for login '.MounirApiAccess::$user->login);
        }
    ...
    public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $sqlfilters = '')
    {
        ...
        if(! MounirApiAccess::$user->rights->bbb->read) {
            throw new RestException(401);
        }

        $socid = MounirApiAccess::$user->societe_id ? MounirApiAccess::$user->societe_id : '';

        $restrictonsocid = 0;	// Set to 1 if there is a field socid in table of object

        // If the internal user must only see his customers, force searching by him
        $search_sale = 0;
        if ($restrictonsocid && ! MounirApiAccess::$user->rights->societe->client->voir && !$socid) $search_sale = MounirApiAccess::$user->id;

        $sql = "SELECT t.rowid";
        if ($restrictonsocid && (!MounirApiAccess::$user->rights->societe->client->voir && !$socid) || $search_sale > 0) $sql .= ", sc.fk_soc, sc.fk_user"; // We need these fields in order to filter by sale (including the case where the user can only see his prospects)
        $sql.= " FROM ".MAIN_DB_PREFIX.$tmpobject->table_element." as t";

        if ($restrictonsocid && (!MounirApiAccess::$user->rights->societe->client->voir && !$socid) || $search_sale > 0) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc"; // We need this table joined to the select in order to filter by sale
        $sql.= " WHERE 1 = 1";
        ...
        if ($restrictonsocid && (!MounirApiAccess::$user->rights->societe->client->voir && !$socid) || $search_sale > 0) $sql.= " AND t.fk_soc = sc.fk_soc";
    ...
    {
        if(! MounirApiAccess::$user->rights->mymodule->write) {
            throw new RestException(401);
        }
        ...
        if( ! $this->myobject->create(MounirApiAccess::$user)) {
    ...
    public function put($id, $request_data = null)
    {
        ...
        if ($this->myobject->update($id, MounirApiAccess::$user) > 0)
        {
            return $this->get($id);
        }
        else
    ...
    {
        if (! MounirApiAccess::$user->rights->mymodule->delete) {
            throw new RestException(401);
        }
        $result = $this->myobject->fetch($id);
        if (! $result) {
        ...
        if (! $this->myobject->delete(MounirApiAccess::$user))
        {
