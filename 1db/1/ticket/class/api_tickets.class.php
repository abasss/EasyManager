 ...
 */
class Tickets extends MounirApi
{
    /**
    ...
    {
        if (! MounirApiAccess::$user->rights->ticket->read) {
        ...
        if (! MounirApi::_checkAccessToResource('ticket', $this->ticket->id)) {
    ...
    public function index($socid = 0, $sortfield = "t.rowid", $sortorder = "ASC", $limit = 100, $page = 0, $sqlfilters = '')
    {
        ...
        if (!$socid && MounirApiAccess::$user->societe_id) {
            $socid = MounirApiAccess::$user->societe_id;
        }

        // If the internal user must only see his customers, force searching by him
        if (! MounirApiAccess::$user->rights->societe->client->voir && !$socid) {
            $search_sale = MounirApiAccess::$user->id;
        }

        $sql = "SELECT t.rowid";
        if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socid) || $search_sale > 0) {
    ...
    public function post($request_data = null)
    {
        ...
        if (! MounirApiAccess::$user->rights->ticket->write) {
        ...
        if ($this->ticket->create(MounirApiAccess::$user) < 0) {
    ...
    public function postNewMessage($request_data = null)
    {
        ...
        if (! $this->ticket->createTicketMessage(MounirApiAccess::$user)) {
    ...
    public function put($id, $request_data = null)
    {
        ...
        if ($this->ticket->update($id, MounirApiAccess::$user)) {
            return $this->get($id);
        }
    ...
    {
        if (! MounirApiAccess::$user->rights->ticket->delete) {
            throw new RestException(401);
        }
        $result = $this->ticket->fetch($id);
        if (! $result) {
