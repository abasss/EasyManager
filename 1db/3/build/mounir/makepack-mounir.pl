#!/usr/bin/perl
#----------------------------------------------------------------------------
# \file         build/makepack-mounir.pl
# \brief        ERP package builder (tgz, zip, rpm, deb, exe, aps)
# \author       (c)2004-2015 Laurent Destailleur  <eldy@users.sourceforge.net>
#
# This is list of constant you can set to have generated packages moved into a specific dir: 
#DESTIBETARC='/media/HDDATA1_LD/Mes Sites/Web/ERP/mounir.org/files/lastbuild'
#DESTISTABLE='/media/HDDATA1_LD/Mes Sites/Web/ERP/mounir.org/files/stable'
...
$PROJECT="mounir";
$PUBLISHSTABLE="eldy,mounir\@frs.sourceforge.net:/home/frs/project/mounir";
$PUBLISHBETARC="mounir\@vmprod1.mounir.org:/home/mounir/mounir.org/httpdocs/files";
...
if (! $ENV{"DESTIBETARC"} || ! $ENV{"DESTISTABLE"})
{
	...
	print "Example: DESTIBETARC='/media/HDDATA1_LD/Mes Sites/Web/ERP/mounir.org/files/lastbuild'\n";
	print "Example: DESTISTABLE='/media/HDDATA1_LD/Mes Sites/Web/ERP/mounir.org/files/stable'\n";
	print "$PROG.$Extension aborted.\n";
	sleep 2;
	exit 1;
}
...
if ($nboftargetok) {
	...
	if ($nbofpublishneedchangelog)
	{
		...
		{
			print 'cd ~/git/mounir_'.$MAJOR.'.'.$MINOR.'; git log `git rev-list --boundary '.$MAJOR.'.'.$MINOR.'..origin/develop | grep ^- | cut -c2- | head -n 1`.. --no-merges --pretty=short --oneline | sed -e "s/^[0-9a-z]* //" | grep -e \'^FIX\|NEW\|CLOSE\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\' | sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
		}
		else			# For a maintenance release
		{
			#print 'cd ~/git/mounir_'.$MAJOR.'.'.$MINOR.'; git log '.$MAJOR.'.'.$MINOR.'.'.($BUILD-1).'.. --no-merges --pretty=short --oneline | sed -e "s/^[0-9a-z]* //" | grep -e \'^FIX\|NEW\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\'| sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
			print 'cd ~/git/mounir_'.$MAJOR.'.'.$MINOR.'; git log '.$MAJOR.'.'.$MINOR.'.'.($BUILD-1).'.. | grep -v "Merge branch" | grep -v "Merge pull" | grep "^ " | sed -e "s/^[0-9a-z]* *//" | grep -e \'^FIX\|NEW\|CLOSE\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\' | sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
		}
		print "\n";
		if (! $ret)
		{
	...
	if ($nboftargetneedbuildroot)
	{
		...
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir_*.deb`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir_*.dsc`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir_*.tar.gz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir_*.tar.xz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir-*.deb`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir-*.rpm`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir-*.tar`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir-*.tar.gz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir-*.tar.xz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir-*.tgz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir-*.xz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/mounir-*.zip`;
		...
		$ret=`rm -f  $BUILDROOT/$PROJECT/dev/mounir_changes.txt`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/dev/README`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot2.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot3.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot4.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot5.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot6.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot7.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot8.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot9.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot10.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot11.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/mounir_screenshot12.png`;
	...
	foreach my $target (sort keys %CHOOSEDTARGET) 
	{
		...
		if ($target =~ /RPM/)	                 # Linux only 
		{
			...
			$ret=`cp "$SOURCE/build/rpm/mounir-forrpm.patch" "$RPMDIR/SOURCES"`;
			$ret=`chmod 644 $RPMDIR/SOURCES/mounir-forrpm.patch`;
		...
		if ($target eq 'DEB') 
		{
			...
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/mounir.config`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/mounir.desktop`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/mounir.docs`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/mounir.install`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/mounir.lintian-overrides`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/mounir.postrm`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/mounir.postinst`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/mounir.templates`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/mounir.templates.futur`;
			...
			$ret=`cp -f  "$SOURCE/build/debian/mounir.desktop"        	"$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/mounir.docs"        		"$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/mounir.install" 	        "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/mounir.lintian-overrides"  "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/mounir.xpm"  		      	"$BUILDROOT/$PROJECT.tmp/debian"`;
			...
			$ret=`cp -f  "$SOURCE/build/debian/mounir.config"         "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/mounir.postinst"       "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/mounir.postrm"         "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/mounir.templates"      "$BUILDROOT/$PROJECT.tmp/debian"`;
	...
	foreach my $target (sort keys %CHOOSEDPUBLISH) 
	{
		...
		%filestoscansf=(
			...
			"$DESTI/package_rpm_generic/$FILENAMERPM"=>'ERP installer for Fedora-Redhat-Mandriva-Opensuse (DoliRpm)',
			"$DESTI/package_rpm_generic/$FILENAMERPMSRC"=>'none',						# none means it won't be published on SF
			"$DESTI/package_debian-ubuntu/${FILENAMEDEB}_all.deb"=>'ERP installer for Debian-Ubuntu (DoliDeb)',
			...
			"$DESTI/package_windows/$FILENAMEEXEDOLIWAMP.exe"=>'ERP installer for Windows (DoliWamp)',
			"$DESTI/standard/$FILENAMETGZ.tgz"=>'ERP ERP-CRM',
			"$DESTI/standard/$FILENAMETGZ.zip"=>'ERP ERP-CRM'
		);
		...
		if ($target eq 'ASSO' && $BUILD =~ /[a-z]/i)   { 	# Not stable
			%filestoscansf=(
				"$DESTI/$FILENAMERPM"=>'ERP installer for Fedora-Redhat-Mandriva-Opensuse (DoliRpm)',
				"$DESTI/${FILENAMEDEB}_all.deb"=>'ERP installer for Debian-Ubuntu (DoliDeb)',
				"$DESTI/$FILENAMEEXEDOLIWAMP.exe"=>'ERP installer for Windows (DoliWamp)',
				"$DESTI/$FILENAMETGZ.tgz"=>'ERP ERP-CRM',
				"$DESTI/$FILENAMETGZ.zip"=>'ERP ERP-CRM'
			);
			%filestoscanstableasso=(
				"$DESTI/$FILENAMERPM"=>'',
		...
		if ($target eq 'SF' || $target eq 'ASSO') 
		{
			...
			foreach my $file (sort keys %filestoscan)
			{
				...
				#$command="ssh eldy,mounir\@frs.sourceforge.net mkdir -p \"$destFolder\"";
				#print "$command\n";	
				#my $ret=`$command 2>&1`;
