...
/**
 * \file scripts/user/sync_groups_ldap2mounir.php
 * \ingroup ldap member
 * \brief Script to update groups into ERP from LDAP
 */
...
if (empty($conf->global->LDAP_GROUP_DN)) {
	print $langs->trans("Error") . ': ' . $langs->trans("LDAP setup for groups not defined inside ERP");
	exit(- 1);
}
...
if ($result >= 0) {
	...
	if (is_array($ldaprecords)) {
		...
		foreach ($ldaprecords as $key => $ldapgroup) {
			...
			// 1 - Association des utilisateurs du groupe LDAP au groupe ERP
			$userList = array();
			$userIdList = array();
			...
			// 2 - Suppression des utilisateurs du groupe ERP qui ne sont plus dans le groupe LDAP
