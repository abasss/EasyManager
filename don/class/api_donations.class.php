 ...
 */
class Donations extends MounirApi
{
    ...
    {
		if(! MounirApiAccess::$user->rights->don->lire) {
		...
		if( ! MounirApi::_checkAccessToResource('don', $this->don->id)) {
    ...
    public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $thirdparty_ids = '', $sqlfilters = '')
    {
        ...
        $sql = "SELECT t.rowid";
        if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socids) ) $sql .= ", sc.fk_soc, sc.fk_user"; // We need these fields in order to filter by sale (including the case where the user can only see his prospects)
        $sql.= " FROM ".MAIN_DB_PREFIX."don as t";

        $sql.= ' WHERE t.entity IN ('.getEntity('don').')';
        if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socids) ) $sql.= " AND t.fk_soc = sc.fk_soc";
    ...
    {
        if (! MounirApiAccess::$user->rights->don->creer) {
        ...
        if ($this->don->create(MounirApiAccess::$user) < 0) {
    ...
    public function put($id, $request_data = null)
    {
		...
		if (! MounirApi::_checkAccessToResource('donation', $this->don->id)) {
			throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
		}
        foreach ($request_data as $field => $value) {
        ...
        if ($this->don->update(MounirApiAccess::$user) > 0)
        {
            return $this->get($id);
        }
        else
    ...
    {
        if(! MounirApiAccess::$user->rights->don->supprimer) {
			throw new RestException(401);
		}
        $result = $this->don->fetch($id);
        if( ! $result ) {
        ...
        if( ! $this->don->delete(MounirApiAccess::$user)) {
    ...
    public function validate($id, $idwarehouse = 0, $notrigger = 0)
    {
		...
		$result = $this->don->valid(MounirApiAccess::$user, $idwarehouse, $notrigger);
		if ($result == 0) {
