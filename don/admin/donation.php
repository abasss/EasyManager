...
{
	if (mounir_set_const($db, "DON_ADDON_MODEL", $value, 'chaine', 0, '', $conf->entity))
	{
...
elseif ($action == 'del')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->DON_ADDON_MODEL == "$value") mounir_del_const($db, 'DON_ADDON_MODEL', $conf->entity);
	}
}
...
if ($action == 'set_DONATION_ACCOUNTINGACCOUNT')
{
    ...
    $res = mounir_set_const($db, "DONATION_ACCOUNTINGACCOUNT", $account, 'chaine', 0, '', $conf->entity);
...
if ($action == 'set_DONATION_MESSAGE')
{
    ...
    $res = mounir_set_const($db, "DONATION_MESSAGE", $freemessage, 'chaine', 0, '', $conf->entity);
...
{
    $code=$reg[1];
    if (mounir_set_const($db, $code, 1, 'chaine', 0, '', $conf->entity) > 0)
    {
