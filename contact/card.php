...
else
{
    ...
    if ($user->rights->societe->contact->creer)
    {
        ...
        elseif ($action == 'edit' && ! empty($id))
        {
            ...
            $doleditor = new DolEditor('note_public', $object->note_public, '', 80, 'mounir_notes', 'In', 0, false, true, ROWS_3, '90%');
            print $doleditor->Create(1);
            print '</td></tr>';

            // Note Private
            print '<tr><td class="tdtop"><label for="note_private">'.$langs->trans("NotePrivate").'</label></td><td colspan="3">';
            $doleditor = new DolEditor('note_private', $object->note_private, '', 80, 'mounir_notes', 'In', 0, false, true, ROWS_3, '90%');
            print $doleditor->Create(1);
            print '</td></tr>';
            ...
            // Login ERP
            print '<tr><td>'.$langs->trans("MounirLogin").'</td><td colspan="3">';
            if ($object->user_id)
            {
                $mounir_user=new User($db);
                $result=$mounir_user->fetch($object->user_id);
                print $mounir_user->getLoginUrl(1);
            }
    ...
    if (! empty($id) && $action != 'edit' && $action != 'create')
    {
        ...
        if ($action == 'create_user')
        {
            ...
            print $form->formconfirm($_SERVER["PHP_SELF"]."?id=".$object->id, $langs->trans("CreateMounirLogin"), $text, "confirm_create_user", $formquestion, 'yes');
        }
		...
		if (empty($reshook) && $action!='presend')
		{
            ...
            {
                print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&action=create_user">'.$langs->trans("CreateMounirLogin").'</a>';
            }
