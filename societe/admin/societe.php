...
{
	if (mounir_set_const($db, "SOCIETE_CODECLIENT_ADDON", $value, 'chaine', 0, '', $conf->entity) > 0)
	{
...
{
	if (mounir_set_const($db, "SOCIETE_CODECOMPTA_ADDON", $value, 'chaine', 0, '', $conf->entity) > 0)
	{
		header("Location: ".$_SERVER["PHP_SELF"]);
		exit;
	}
...
if ($action == 'updateoptions')
{
	if (GETPOST('COMPANY_USE_SEARCH_TO_SELECT'))
	{
		...
		$res = mounir_set_const($db, "COMPANY_USE_SEARCH_TO_SELECT", $companysearch, 'chaine', 0, '', $conf->entity);
		if (! $res > 0) $error++;
		if (! $error)
	    {
	...
	if (GETPOST('CONTACT_USE_SEARCH_TO_SELECT'))
	{
		...
		$res = mounir_set_const($db, "CONTACT_USE_SEARCH_TO_SELECT", $contactsearch, 'chaine', 0, '', $conf->entity);
		if (! $res > 0) $error++;
		if (! $error)
		{
			setEventMessages($langs->trans("SetupSaved"), null, 'mesgs');
	...
	if (GETPOST('THIRDPARTY_CUSTOMERTYPE_BY_DEFAULT'))
	{
		...
		$res = mounir_set_const($db, "THIRDPARTY_CUSTOMERTYPE_BY_DEFAULT", $customertypedefault, 'chaine', 0, '', $conf->entity);
		if (! $res > 0) $error++;
		if (! $error)
		{
			setEventMessages($langs->trans("SetupSaved"), null, 'mesgs');
...
if ($action == 'setdoc')
{
	...
	mounir_set_const($db, "COMPANY_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity);
...
if ($action=="setaddrefinlist") {
	...
	$res = mounir_set_const($db, "SOCIETE_ADD_REF_IN_LIST", $setaddrefinlist, 'yesno', 0, '', $conf->entity);
	if (! $res > 0) $error++;
	if (! $error)
	{
		setEventMessages($langs->trans("SetupSaved"), null, 'mesgs');
...
if ($action=="setaddadressinlist") {
	...
	$res = mounir_set_const($db, "COMPANY_SHOW_ADDRESS_SELECTLIST", $val, 'yesno', 0, '', $conf->entity);
	if (! $res > 0) $error++;
	if (! $error)
	{
		setEventMessages($langs->trans("SetupSaved"), null, 'mesgs');
...
if ($action=="setaskforshippingmet") {
	...
	$res = mounir_set_const($db, "SOCIETE_ASK_FOR_SHIPPING_METHOD", $setaskforshippingmet, 'yesno', 0, '', $conf->entity);
	if (! $res > 0) $error++;
	if (! $error)
	{
		setEventMessages($langs->trans("SetupSaved"), null, 'mesgs');
...
if ($action=="setdisableprospectcustomer") {
    ...
    $res = mounir_set_const($db, "SOCIETE_DISABLE_PROSPECTSCUSTOMERS", $setdisableprospectcustomer, 'yesno', 0, '', $conf->entity);
    if (! $res > 0) $error++;
    if (! $error)
    {
        setEventMessages($langs->trans("SetupSaved"), null, 'mesgs');
...
if ($action == 'setprofid')
{
	...
	if (mounir_set_const($db, $idprof, $status, 'chaine', 0, '', $conf->entity) > 0)
	{
		//header("Location: ".$_SERVER["PHP_SELF"]);
		//exit;
	}
...
if ($action == 'sethideinactivethirdparty')
{
	...
	if (mounir_set_const($db, "COMPANY_HIDE_INACTIVE_IN_COMBOBOX", $status, 'chaine', 0, '', $conf->entity) > 0)
	{
		header("Location: ".$_SERVER["PHP_SELF"]);
		exit;
	}
...
if($action=='setonsearchandlistgooncustomerorsuppliercard'){
       ...
       $res = mounir_set_const($db, "SOCIETE_ON_SEARCH_AND_LIST_GO_ON_CUSTOMER_OR_SUPPLIER_CARD", $setonsearchandlistgooncustomerorsuppliercard, 'yesno', 0, '', $conf->entity);
       if (! $res > 0) $error++;
       if (! $error)
       {
               setEventMessages($langs->trans("SetupSaved"), null, 'mesgs');
