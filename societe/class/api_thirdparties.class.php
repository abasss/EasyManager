 ...
 *
 */
class Thirdparties extends MounirApi
{
	/**
	 *
	...
	{
		if(! MounirApiAccess::$user->rights->societe->lire) {
		...
		if( ! MounirApi::_checkAccessToResource('societe', $this->company->id)) {
    ...
    public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $mode = 0, $sqlfilters = '')
    {
		...
		$socids = MounirApiAccess::$user->societe_id ? MounirApiAccess::$user->societe_id : '';
		...
		if ((!MounirApiAccess::$user->rights->societe->client->voir && !$socids) || $search_sale > 0) $sql.= " AND t.rowid = sc.fk_soc";
	...
	{
		if(! MounirApiAccess::$user->rights->societe->creer) {
			throw new RestException(401);
		}
		...
		if ($this->company->create(MounirApiAccess::$user) < 0)
    ...
    public function put($id, $request_data = null)
	{
		...
		if($this->company->update($id, MounirApiAccess::$user, 1, '', '', 'update'))
			return $this->get($id);
    ...
    public function merge($id, $idtodelete)
	{
		...
		if( ! MounirApi::_checkAccessToResource('societe', $this->companytoremove->id)) {
			throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
		}

		$soc_origin = $this->companytoremove;
		$object = $this->company;
		$user = MounirApiAccess::$user;
...
$reshook = $hookmanager->executeHooks('replaceThirdparty', array(
	...
	{
		if(! MounirApiAccess::$user->rights->societe->supprimer) {
			throw new RestException(401);
		}
		$result = $this->company->fetch($id);
		if( ! $result ) {
    ...
    public function addCategory($id, $category_id)
	{
		...
		if( ! MounirApi::_checkAccessToResource('category', $category->id)) {
			throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
		}
    ...
    public function getOutStandingProposals($id, $mode = 'customer')
	{
		...
		if( ! MounirApi::_checkAccessToResource('societe', $id)) {
			throw new RestException(401, 'Access not allowed for login '.MounirApiAccess::$user->login);
		}
    ...
    public function createCompanyBankAccount($id, $request_data = null)
	{
		...
		if ($account->create(MounirApiAccess::$user) < 0)
			throw new RestException(500, 'Error creating Company Bank account');


		if ($account->update(MounirApiAccess::$user) < 0)
    ...
    public function deleteCompanyBankAccount($id, $bankaccount_id)
    {
		...
		return $account->delete(MounirApiAccess::$user);
	}
	...
	public function generateBankAccountDocument($id, $companybankid = null, $model = 'sepamandate')
	{
		...
		$this->company->setDocModel(MounirApiAccess::$user, $model);
  ...
  /**
	 ...
	 * See <a href="https://www.google.com/index.php/Table_llx_societe_account">Table llx_societe_account</a> wiki page for more information<br><br>
    ...
    public function deleteSocieteAccount($id, $site)
    {
		...
		if($result->num_rows == 0 ){
			...
			if($account->delete(MounirApiAccess::$user) < 0) {
